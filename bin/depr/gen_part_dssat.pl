...
our($num_scenarios, $years, $variables, $crop, $grid, $gridDir, $outfile);
GetOptions (
...
$grid = basename($grid);
my $summary_file="data/Summary.OUT";
...
my %column_names = ();
my @column_definitions = split(/\s+/, $summary_data[3]);
...
# There are $years*$num_scenarios grids, and a header length of 4
foreach my $line_number(0..(($years*$num_scenarios)-1+4)) {
   ...
   # Handle missing data
   if(scalar(@summary_data) < $line_number) { $row = 'NaN' x 52; }
   ...
   foreach my $variable(@variable_list) {
      ...
      if(defined($accumulated_values{$file_to_create})) {
         ...
         $accumulated_values{ $file_to_create } = "$file_to_create,$grid,$gridDir,$columns[$column_names{$variable}]";
      }
   }
