...
Begin
ProcessNext (SoilWat.nLayers, 'Number of layers for soil water', 7, CommentStr, VarStr, ReadWrite, Defp);
...
Var Defp: text;
    ...
    DecayDummy: Real48;
        ...
        var iLayer: Integer;
        Begin
        RealIn(SoilOrganic.OMTransfer[1], 'Annual transfer of organic matter from one layer to the next lower layer');
        If CommentStr = 'Annual transfer of organic matter from one layer to the next lower layer' then
           For iLayer := 2 to SoilWat.nLayers do
               SoilOrganic.OMTransfer[iLayer] := SoilOrganic.OMTransfer[1];
...
while not eof(defp) do
      ...
      RealIn (SoilOrganic.OMTransfer[iLayer], 'Organic matter transfer rate in the soil');
      ...
      OldOMTransferIn;{(SoilOrganic.OMTransfer[1], 'Annual transfer of organic matter from one layer to the next lower layer');} // For reading data from an old file
...
Procedure SaveSite(name : string);
    ...
    For i := 1 to SoilWat.nLayers do
        ...
        LineOut(SoilOrganic.OMTransfer[i], ' Organic matter transfer rate in the soil');
