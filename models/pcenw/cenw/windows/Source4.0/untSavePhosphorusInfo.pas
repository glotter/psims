{ ================================================================
  = Project   : CenW                                             =
  ================================================================
  = Modules   : TfrmSavePhosphorusInfo                           =
  =                                                              =
  =             Interface routine to nominate phosphorus-related =
  =             variables that are to be saved in an ASCII file  =
  =             during program execution.                        =
  ================================================================
  = File      : untSavePhosphorusInfo.PAS                        =
  =                                                              =
  = Version   : 4.0                                              =
  ================================================================ }

unit untSavePhosphorusInfo;
...
type
  TfrmSavePhosphorusInfo = class(TForm)
    btnOK: TBitBtn;
    btnCancel: TBitBtn;
    btnHelp: TBitBtn;
...
var
  frmSavePhosphorusInfo: TfrmSavePhosphorusInfo;
...
Procedure TfrmSavePhosphorusInfo.FormShow(Sender: TObject);
var
  cbEnabled: TCheckBox;
...
begin
  ...
  End;
  iEntries := 0;
  For SaveInFileVar := S_SolubleP to S_TotalP do
      iEntries := iEntries + 1;
  If iEntries <= 20 then
     Begin
     frmSavePhosphorusInfo.ClientWidth := 260;
     End
  Else
     Begin
     frmSavePhosphorusInfo.ClientWidth := 400;
     End;
  If iEntries > 20 then
      HalfEntries := (iEntries + 1) div 2
  Else
      HalfEntries := iEntries;
  frmSavePhosphorusInfo.Height := 120 + (HalfEntries + 4) * 16;
  btnOK.Top := frmSavePhosphorusInfo.Height - 60;
...
Procedure TfrmSavePhosphorusInfo.btnOKClick(Sender: TObject);
var
  iCount: integer;
  SaveInFileVar: SaveVariableOptions;
begin
...
Procedure TfrmSavePhosphorusInfo.rgSaveGenericClick(Sender: TObject);
var SaveInFileVar: SaveVariableOptions;
begin
frmSavePhosphorusInfo.btnOKClick(Sender);
...
Else if rgSaveGeneric.ItemIndex = 1 then
   ...
   Control.ProjectHasChanged := TRUE;
   End;
frmSavePhosphorusInfo.FormShow(Sender);
