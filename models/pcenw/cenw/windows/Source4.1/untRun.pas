...
uses
  Forms, SysUtils, Windows, Math, untDeclarations, untSimulate, untSimSoil, untParFitProgress,
  untFileIO, untDiskOut, untMiscellaneous, untDivideValidation, untProcessFittingData,
...
If (Control.EquilMode or Control.Parfit.FittingMode) and (not Control.Run_on or (frmGraph = nil)) then
...
If Control.OutputFileOpen and ((not Control.BatchMode) or Control.EndBatch) and
   (not Control.EquilMode) and (not Control.SpatialMode) and (not Control.SensitivityTestOn)
   and (not Control.Parfit.FittingMode) then
   Begin
...
Else
  ...
  try
    While Control.Run_on do
      ...
      if (Control.EquilMode{ or Control.Parfit.FittingMode}) then
         {do nothing yet}
