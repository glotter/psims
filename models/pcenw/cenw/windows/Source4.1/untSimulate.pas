...
uses
  ...
  untTrigValidation, untPowerValidation, untNotices, Messages, Dialogs, Controls, variants;

     Procedure Dilute (var NewPoolC13: Real48; NewPool, OldPoolC13, FluxIn: Real48);
     Procedure InitialiseIteration;
...
Derived.NLeached := 0;
Derived.PLeached := 0;
Derived.GrazerRespiration := 0;
Derived.CH4Flux := 0;
End; {of Procedure 'InitialiseIteration'}

Procedure WeedUnit;
type
CanopyTYpe = Record
             Total, Top, Middle, Bottom: array [CohortTypes] of Real48;
             End;

Var BottomFoliage, CanopyDepth, LowestFoliageOver, PassedThrough, TotalIntercept: Real48;
...
Procedure CarbonGain;
    ...
    Procedure RainDamage(var DamageUnits, RainDamage: real48);
    Begin
...
Derived.NEE := Plant.NewGrowth[C] + Plant.NewWeedGrowth[C] - Derived.GrazerRespiration;
        ...
        For E := C13 to LastE do
          ...
          Else {if Event.HarvestUnits = 'V' or 'W'}
             ...
             If E = C then
                ...
                Else
                   V_Dummy := 1;
                if (Plant.SapWood[C] + Plant.HeartWood[C]) > 0 then
     ...
     If Event.FireTimes[j, 4] = Control.TotalDays then
        ...
        If Control.IncludeIsotopes then
          ...
          SumCIn := Plant.SapWood[C] * Event.WoodToChar[j] + Plant.HeartWood[C] * Event.WoodToChar[j] +
                    ...
                    Soil.CoarseWood[0, C] * Event.WoodToChar[j] + Litter.CoarseWood[C] * Event.WoodToChar[j] +
                    Litter.Leaves[C] * Event.FineToChar[j] + Litter.Other[C] * Event.FineToChar[j] +
                    Litter.FineWood[C] * Event.FineToChar[j] + Soil.FineWood[0, C] * Event.FineToChar[j] +
                    Soil.Struct[0, C] * Event.FineToChar[j] + Soil.Metab[0, C] * Event.FineToChar[j] +
          ...
          SumC13 := Plant.SapWood[C] * Plant.SapWood[C13] * Event.WoodToChar[j] + Plant.HeartWood[C] * Plant.HeartWood[C13] * Event.WoodToChar[j] +
                    ...
                    Soil.CoarseWood[0, C] * Soil.CoarseWood[0, C13] * Event.WoodToChar[j] + Litter.CoarseWood[C] * Litter.CoarseWood[C13] * Event.WoodToChar[j] +
                    Litter.Leaves[C] * Litter.Leaves[C13] * Event.FineToChar[j] + Litter.Other[C] * Litter.Other[C13] * Event.FineToChar[j] +
                    Litter.FineWood[C] * Litter.FineWood[C13] * Event.FineToChar[j] + Soil.FineWood[0, C] * Soil.FineWood[0, C13] * Event.FineToChar[j] +
                    Soil.Struct[0, C] * Soil.Struct[0, C13] * Event.FineToChar[j] + Soil.Metab[0, C] * Soil.Metab[0, C13] * Event.FineToChar[j] +
          ...
          Dilute (Soil.Inert[0, C13], Soil.Inert[0, C], DummyC13, SumCIn);
        ...
        For E := C to LastE do
          ...
          Soil.Inert[0, E] := Soil.Inert[0, E] * (1 - Event.LitterBurn[j] * CombustionRatio) +
                                    (Plant.Sapwood[E] + Plant.HeartWood[E] + Plant.Bark[E] + Plant.Branches[E] + Soil.CoarseWood[0, E] + Litter.CoarseWood[E]) * CombustionRatio * Event.WoodToChar[j] +
                                    (Litter.Leaves[E] + Litter.Other[E] + Litter.FineWood[E] + Soil.FineWood[0, E] + Soil.Struct[0, E] + Soil.Metab[0, E] +
                                     Soil.Active[0, E] + Soil.Slow[0, E] + Soil.Resistant[0, E] + Soil.Soluble[0, E]
          ...
          Soil.FineWood[0, E] := Soil.FineWood[0, E] * (1 - Event.LitterBurn[j] * CombustionRatio);
          Soil.CoarseWood[0, E] := Soil.CoarseWood[0, E] * (1 - Event.LitterBurn[j] * CombustionRatio);
          Soil.Struct[0, E] := Soil.Struct[0, E] * (1 - Event.LitterBurn[j] * CombustionRatio);
          Soil.Metab[0, E] := Soil.Metab[0, E] * (1 - Event.LitterBurn[j] * CombustionRatio);
          Soil.Active[0, E] := Soil.Active[0, E] * (1 - Event.LitterBurn[j] * CombustionRatio);
          Soil.Slow[0, E] := Soil.Slow[0, E] * (1 - Event.LitterBurn[j] * CombustionRatio);
          Soil.Resistant[0, E] := Soil.Resistant[0, E] * (1 - Event.LitterBurn[j] * CombustionRatio);
          Soil.Soluble[0, E] := Soil.Soluble[0, E] * (1 - Event.LitterBurn[j] * CombustionRatio);
...
for iLayer := 1 to Soil.nLayers do
    Derived.Disturbed[iLayer] := 1 + Event.PloughDisturbanceRecovery * (Derived.Disturbed[iLayer] - 1);
If j <= Event.nPloughing then
     If (Event.PloughTimes[j, 4] = Control.TotalDays) and (Event.PloughDepth[j] > 0) then
        ...
        For iLayer := 1 to Event.PloughDepth[j] do
            Begin
            TotalDepth := TotalDepth + Soil.WaterLayer[iLayer].Depth;
            Derived.Disturbed[iLayer] := Derived.Disturbed[iLayer] * Event.Disturbance[j];
            End;
        SumLignin := 0;
        SumStemLignin := 0;
        If Control.IncludeP then
        ...
        For E := FirstE to LastE do
            ...
            For iLayer := 0 to Event.PloughDepth[j] do
               If E <> C13 then
                  Begin
                  SumStruct[E] := SumStruct[E] + Soil.Struct[iLayer, E]
                  ...
                  SumMetab[E] := SumMetab[E] + Soil.Metab[iLayer, E];
                  SumActive[E] := SumActive[E] + Soil.Active[iLayer, E];
                  SumSlow[E] := SumSlow[E] + Soil.Slow[iLayer, E];
                  SumResistant[E] := SumResistant[E] + Soil.Resistant[iLayer, E];
                  SumInert[E] := SumInert[E] + Soil.Inert[iLayer, E];
                  If E = C then
                     SumLignin := SumLignin + Soil.LitterLig[iLayer] * Soil.Struct[iLayer, C]
                                + Soil.BranchLig[iLayer] * Soil.FineWood[iLayer, C];
                  End
               Else
                  Begin
                  SumStruct[C13] := SumStruct[C13] + Soil.Struct[iLayer, C] * Soil.Struct[iLayer, C13]
                                                   + Soil.FineWood[iLayer, C] * Soil.FineWood[iLayer, C13];
                  SumMetab[C13] := SumMetab[C13] + Soil.Metab[iLayer, C] * Soil.Metab[iLayer, C13];
                  SumActive[C13] := SumActive[C13] + Soil.Active[iLayer, C] * Soil.Active[iLayer, C13];
                  SumSlow[C13] := SumSlow[C13] + Soil.Slow[iLayer, C] * Soil.Slow[iLayer, C13];
                  SumResistant[C13] := SumResistant[C13] + Soil.Resistant[iLayer, C] * Soil.Resistant[iLayer, C13];
                  SumInert[C13] := SumInert[C13] + Soil.Inert[iLayer, C] * Soil.Inert[iLayer, C13];
                  End;
            For iLayer := 1 to Event.PloughDepth[j] do
                Begin
                Soil.Struct[iLayer, E] := SumStruct[E] * Soil.WaterLayer[iLayer].Depth / TotalDepth;
                Soil.Metab[iLayer, E] := SumMetab[E] * Soil.WaterLayer[iLayer].Depth / TotalDepth;
                Soil.Active[iLayer, E] := SumActive[E] * Soil.WaterLayer[iLayer].Depth / TotalDepth;
                Soil.Slow[iLayer, E] := SumSlow[E] * Soil.WaterLayer[iLayer].Depth / TotalDepth;
                Soil.Resistant[iLayer, E] := SumResistant[E] * Soil.WaterLayer[iLayer].Depth / TotalDepth;
                Soil.Inert[iLayer, E] := SumInert[E] * Soil.WaterLayer[iLayer].Depth / TotalDepth;
                If E = C then
                   Begin
                   AdjustProperty (Soil.LitterLig[iLayer], SumLignin, SumStruct[C]);
                   AdjustProperty (Soil.Struct[iLayer, C13], SumStruct[C13], SumStruct[C]);
                   AdjustProperty (Soil.Metab[iLayer, C13], SumMetab[C13], SumStruct[C]);
                   AdjustProperty (Soil.Active[iLayer, C13], SumActive[C13], SumStruct[C]);
                   AdjustProperty (Soil.Slow[iLayer, C13], SumSlow[C13], SumStruct[C]);
                   AdjustProperty (Soil.Resistant[iLayer, C13], SumResistant[C13], SumStruct[C]);
                   AdjustProperty (Soil.Inert[iLayer, C13], SumInert[C13], SumInert[C]);
                   End;
                End;
            Soil.FineWood[0, E] := 0;
            Soil.Struct[0, E] := 0;
...
Procedure CarbonLoss;
    ...
    If (Parameter.MortalityType = Density) or (Parameter.MortalityType = Both) then
       Begin
       MaxWeight := (exp(Parameter.Three_Two_Power_Law) * Power(Plant.Stocking, -Parameter.Three_Two_Power_Law_Slope)) / Control.CConversion;
    ...
    Litter.FineRoot[C] := Plant.FineRoot[C] * (Parameter.RootSenesc + Derived.CLossMortality + Parameter.RootDesicSensitivity * Derived.Desic);
    ...
    Plant.Height := Plant.Height * Power(MeanRatio,
                   Divide(1, (Parameter.WHSlope + Divide(Parameter.WDSlope, Derived.HDSlope))));
    Plant.DBH := Plant.DBH * Power(MeanRatio,
                   Divide(1, (Derived.HDSlope * Parameter.WHSlope + Parameter.WDSlope)));
...
Litter.FineRoot[N] := Plant.FineRoot[N] * (Parameter.RootSenesc + Derived.CLossMortality + Parameter.RootDesicSensitivity * Derived.Desic);
...
Var NewNLimit, NRatio, Vegetative, AllocSum, ActLBRatio, InitialNonWoodAlloc, AdjustRatio: real48;
   ...
   Derived.N_LeafAlloc := Divide(Derived.N_LeafAlloc, AllocSum);
   Derived.N_SapWoodAlloc := Divide(Derived.N_SapWoodAlloc, AllocSum);
   Derived.N_BranchAlloc := Divide(Derived.N_BranchAlloc, AllocSum);
   Derived.N_BarkAlloc := Divide(Derived.N_BarkAlloc, AllocSum);
   Derived.N_FineRootAlloc := Divide(Derived.N_FineRootAlloc, AllocSum);
   Derived.N_CoarseRootAlloc := Divide(Derived.N_CoarseRootAlloc, AllocSum);
   Derived.N_FruitAlloc := Divide(Derived.N_FruitAlloc, AllocSum);
   Derived.N_PollenAlloc := Divide(Derived.N_PollenAlloc, AllocSum);
   if Control.IncludeP then
      Begin
      ...
      Derived.P_LeafAlloc := Divide(Derived.P_LeafAlloc, AllocSum);
      Derived.P_SapWoodAlloc := Divide(Derived.P_SapWoodAlloc, AllocSum);
      Derived.P_BranchAlloc := Divide(Derived.P_BranchAlloc, AllocSum);
      Derived.P_BarkAlloc := Divide(Derived.P_BarkAlloc, AllocSum);
      Derived.P_FineRootAlloc := Divide(Derived.P_FineRootAlloc, AllocSum);
      Derived.P_CoarseRootAlloc := Divide(Derived.P_CoarseRootAlloc, AllocSum);
      Derived.P_FruitAlloc := Divide(Derived.P_FruitAlloc, AllocSum);
      Derived.P_PollenAlloc := Divide(Derived.P_PollenAlloc, AllocSum);
      End;
...
var NewCStruct, NewNStruct, NewPStruct, KmC, KmN, AllC, AllN, MaxN,
    ...
    GrowVolume, PowerStart,
    dVdH, alpha, A, dHdV, GrowDg}: real48;
    ...
    Procedure CalcNewHeightandDiameter;
    var alpha, A, dVdH, GrowVolume, NewWood, OldVolume, DiamGround,
        NewDiamGround: Real48;
        UseAnalytical: Boolean;

        Procedure AnalyticalSolution;  // probably has an error in units due to previous error with inconsistent use of wood density
        Begin
  ...
  //    dHdV := Divide(40000 * Power(A, Divide(2, Derived.HDSlope)), (pi * Plant.Stocking * Parameter.Form)) *
    //          Divide(Power((Plant.Height - 1.3), 3), ((alpha - 2) * Power(Plant.Height, alpha) - 1.3 * alpha * Power(Plant.Height, (alpha - 1))));
        ...
        End; {of Procedure 'AnalyticalSolution'}

        Procedure NumericSolution;
        Const HeightLimit = 2.3;
        var CalcVolume, NewVolume, NewHeight, NewDBH, InitRatio, D1, D2: Real48;
        Begin
        if Plant.Height > HeightLimit then // plants are tall enough to base calculations on previous DBH alone
           Begin
           DiamGround := Plant.DBH * Divide(Plant.Height, (Plant.Height - 1.3)); // old value
           End
        Else // for small plants calculate Diamground more based on plant size rather than previous DBH
           Begin
           OldVolume := (Plant.SapWood[C] + Plant.HeartWood[C]) * Control.CConversion / Derived.GrowthSheathDensity;  // assume all initial growth was at current sheath wood density
           D1 := 200 * Sqrt(Divide((OldVolume), pi * Plant.Height * Plant.Stocking * Parameter.Form));
           D2 := Plant.DBH * Divide(Plant.Height, (Plant.Height - 1.3));
           InitRatio := (HeightLimit - Plant.Height) / (HeightLimit - 1.3);
           DiamGround := D1 * InitRatio + D2 * (1 - InitRatio);
           End;
        OldVolume := Square(DiamGround / 200) * pi * Plant.Height * Plant.Stocking * Parameter.Form;
        NewVolume := OldVolume + GrowVolume;
        NewDBH := Plant.DBH * Power(GrowthRatio, 1/3); // Initial guess
        NewHeight := exp(ln(Plant.Height) + Derived.HDSlope * (ln(NewDBH) - ln(Plant.DBH)));
        if Plant.Height > HeightLimit then // plants are tall enough to base calculations on previous DBH alone
           NewDiamGround := NewDBH * NewHeight / (NewHeight - 1.3)
        Else
           Begin
           D1 := DiamGround * Power(GrowthRatio, 1/3);
           D2 := NewDBH * NewHeight / (NewHeight - 1.3);
           NewDiamGround := D1 * InitRatio + D2 * (1 - InitRatio); // Initial guess
           End;
        Repeat
            CalcVolume := Sqr(NewDiamGround / 200) * pi * NewHeight * Plant.Stocking * Parameter.Form;
            if abs(1 - CalcVolume / NewVolume) > 1e-6 then
               Begin
               NewHeight := NewHeight * Power(NewVolume / CalcVolume, 1/3);
               if NewHeight < Plant.Height then // Make sure plants can't get shorter
                  NewHeight := Plant.Height
               Else if NewHeight > (Plant.Height * GrowthRatio) then // nor can they grow disproportionately
                  NewHeight := Plant.Height * GrowthRatio;
               NewDiamGround := NewDiamGround * Power(NewVolume / CalcVolume, 1/3);
               End;
        Until abs(1 - CalcVolume / NewVolume) < 1e-6;
        Plant.Height := NewHeight;
        Plant.DBH := NewDiamGround * (NewHeight - 1.3) / NewHeight;
        End; {of Procedure 'NumericSolution'}

    Begin
    NewWood := (Plant.SapWood[C] + Plant.HeartWood[C]) * (GrowthRatio - 1);
    GrowVolume := NewWood * Control.CConversion / Derived.GrowthSheathDensity;
    UseAnalytical := false;
    if UseAnalytical then
       AnalyticalSolution
    Else
       NumericSolution;
    End; {of Procedure 'CalcNewHeightandDiameter'}
...
If (KmC > 0) and (KmN > 0) then
    ...
    If NewCStruct > 0.5 * Plant.Soluble[C] then  // maximum of 50% of CH2O can be used for growth on a given day - forces some minor residual storage. Had previously been set to 0.1
       NewCStruct := 0.5 * Plant.Soluble[C];
...
Else
   Derived.Rg := 0;
Derived.DayCFlux := Derived.DayCFlux - (Derived.Rg + Derived.Rm + Derived.GrazerRespiration) * Control.NSeconds / 86400;
Derived.NightCFlux := Derived.NightCFlux - (Derived.Rg + Derived.Rm + Derived.GrazerRespiration) * (86400 - Control.NSeconds) / 86400;
...
If Plant.Stocking * (Plant.SapWood[C] + Plant.HeartWood[C]) > 0 then
   ...
   If Plant.DBH > Parameter.Mindbh then
      ...
      Else
         // Based on explict use of Wood density of the current growth sheeth
          CalcNewHeightandDiameter;
      End
   Else // if Plant.DBH <= Parameter.Mindbh then
      Begin
      If Plant.Height <= 1.3 then
         Begin
         Volume := (Plant.SapWood[C] + Plant.HeartWood[C]) * Control.CConversion /
                   (Derived.WoodDensity * Plant.Stocking);  // m3 tree-1
         ...
         End
      Else if Plant.DBH = 0 then // for the first calculation after height reaches 1.3 m
         Begin
         Volume := (Plant.SapWood[C] + Plant.HeartWood[C]) * Control.CConversion /
                   (Derived.WoodDensity * Plant.Stocking);  // m3 tree-1
         Plant.Height := Power(4 * Sqr(Parameter.Ht_Diameter) * Volume / (pi * Parameter.Form), 1/3);
         ...
         Else
            Plant.DBH := 0;                                      // in case with the new calculations height is less than 1.3
         End
      Else
         CalcNewHeightAndDiameter;
      End;
   Plant.Area := Plant.Stocking * pi * Sqr(Plant.DBH / 2) * 1.05;
        ...
        to stand basal area than to mean stand diameter. This consideration is applied only
        to the ultimate calculation of stanbd basal area but does not affect stand biomass.
...
Var Interception, SnowMelt, WaterIn, WaterRatio, LayerLimit: Real48;
    iLayer: integer;
BEGIN
Derived.All_Litter := 0.001 * Control.CConversion * (Soil.Struct[0, C] + Soil.Metab[0, C] +
              Soil.Active[0, C] + Soil.Slow[0, C] + Soil.Resistant[0, C]);
Soil.WaterLayer[0].MaxWater := Parameter.LitterWHC * Derived.All_Litter * 0.1;
...
If Soil.Snow > 0 then
   ...
   If SnowMelt > 0 then
      Begin
      If SnowMelt > Soil.Snow then
         Begin
         WaterIn := WaterIn + Soil.Snow;
         Soil.Snow := 0;
         End
      Else {if SnowMelt < Soil.Snow then}
         Begin
         WaterIn := WaterIn + SnowMelt;
         Soil.Snow := Soil.Snow - SnowMelt;
         End;
      End;
   End;
...
For iLayer := 0 to Soil.nLayers do
    Begin
    Soil.WaterLayer[iLayer].WaterContent := Soil.WaterLayer[iLayer].WaterContent + WaterIn;
    If Soil.WaterLayer[iLayer].WaterContent > Soil.WaterLayer[iLayer].MaxWater then
       Begin
       WaterIn := Soil.WaterLayer[iLayer].WaterContent - Soil.WaterLayer[iLayer].MaxWater;
       Soil.WaterLayer[iLayer].Percolate := WaterIn;
       Soil.WaterLayer[iLayer].WaterContent := Soil.WaterLayer[iLayer].MaxWater;
       End
    Else
       WaterIn := 0;
    Derived.Drainage[iLayer] := WaterIn;
    If Soil.SeparateSensitivity and (iLayer > 0) then
       Begin
       LayerLimit := Divide(Divide(Soil.WaterLayer[iLayer].WaterContent, Soil.WaterLayer[iLayer].MaxWater), Parameter.StressLimit);
       If LayerLimit > 1 then LayerLimit := 1;
       Derived.WaterLimit := Derived.WaterLimit + Soil.WaterLayer[iLayer].StressSensitivity * LayerLimit;
       End;
    End;
If Not Soil.SeparateSensitivity then
   Derived.WaterLimit := Divide(Divide(Soil.TotalWater, Soil.MaxWater), Parameter.StressLimit);
If Derived.WaterLimit > 1 then
   Derived.WaterLimit := 1;
if Derived.WaterLimit = 1 then   // not waterstressed - check for water-logging
   Begin
   WaterRatio := Divide(Soil.TotalWater, Soil.MaxWater);
   if WaterRatio > Parameter.WaterLogLimit then
      Derived.WaterLimit := (1 - Parameter.WaterLogSensitivity * (WaterRatio - Parameter.WaterLogLimit));
   End;
Derived.BallBerry := (Parameter.BallBerry2 + (Parameter.BallBerry1 - Parameter.BallBerry2) * Derived.WaterLimit);
For iLayer := 0 to Soil.nLayers do
    Begin
    If Soil.WaterLayer[iLayer].MaxWater > 0 then
       WaterRatio :=  Soil.WaterLayer[iLayer].WaterContent / Soil.WaterLayer[iLayer].MaxWater
    Else
       WaterRatio := 1;
...
Var LeafW, DeltaW, A, LitterEvaporation, SoilEvaporation, ActualExtract, WaterRatio,
    LitterFract, Sigma, ra, rc, rs, Rnet, Gamma2, EvapSum: real48;
...
Soil.MaxExtract := 0; ActualExtract := 0; Soil.TotalWater := 0;
If (Control.nSeconds = 0) or (Soil.Snow > 0) then
...
Else
   ...
   If Soil.WaterLayer[0].WaterContent > 1e-6 then
      rs := 800 * (Soil.WaterLayer[0].MaxWater / Soil.WaterLayer[0].WaterContent) // Litter diffusion resistance - loosely equivalent to stomatal resistance
   Else
      rs := 100000;
// Loosely based on Camillo and Gurney (1986) Soil Science 141: 95-105.
   ...
   If Soil.WaterLayer[1].WaterContent > (0.01 * Soil.WaterLayer[1].MaxWater) then
      rs := 800 * (Soil.WaterLayer[1].MaxWater / Soil.WaterLayer[1].WaterContent) // Litter diffusion resistance - loosely equivalent to stomatal resistance
   Else
      rs := 10000;
   SoilEvaporation := (1 - LitterFract) * Control.nSeconds * (Sigma * Rnet + DeltaW * 1e5 * rho * Cp / ra) /
                    (Latent * (Sigma + Gamma2 * (ra + rs) / ra));
   If Soil.WaterLayer[0].MaxWater > 0 then
      Begin
      If LitterEvaporation > Soil.WaterLayer[0].WaterContent then
         LitterEvaporation := Soil.WaterLayer[0].WaterContent;
      End
   Else
      LitterEvaporation := 0;
   Soil.WaterLayer[0].WaterContent := Soil.WaterLayer[0].WaterContent - LitterEvaporation;
   Derived.Evaporation := Derived.Evaporation + LitterEvaporation;
   If SoilEvaporation > 0 then
      Begin
      EvapSum := 0;
      EvapLayer := 1;
      While (Soil.WaterLayer[EvapLayer].RelEvap > 0) do
          Begin
          if (Soil.WaterLayer[EvapLayer].MaxWater > 0) then
             Begin
             Evap[EvapLayer] := Parameter.SoilEvap * Soil.WaterLayer[EvapLayer].RelEvap * Soil.WaterLayer[EvapLayer].WaterContent / Soil.WaterLayer[EvapLayer].MaxWater;
      ...
      If EvapSum > 0 then
         ...
         For iLayer := 1 to EvapLayer - 1 do
             ...
             If Evap[iLayer] < Soil.WaterLayer[iLayer].WaterContent then
                Begin
                Soil.WaterLayer[iLayer].WaterContent := Soil.WaterLayer[iLayer].WaterContent - Evap[iLayer];
                Derived.Evaporation := Derived.Evaporation + Evap[iLayer];
                End
             Else
                Begin
                Derived.Evaporation := Derived.Evaporation + Soil.WaterLayer[iLayer].WaterContent;
...
For iLayer := 1 to Soil.nLayers do
    ...
    If Soil.WaterLayer[iLayer].MaxWater > 0 then
       ...
       WaterRatio :=  Soil.WaterLayer[iLayer].WaterContent / Soil.WaterLayer[iLayer].MaxWater;
       If WaterRatio > Parameter.StressLimit then
          Extract[iLayer] := Soil.WaterLayer[iLayer].ExtractEffort
       else
          Extract[iLayer] := Soil.WaterLayer[iLayer].ExtractEffort * WaterRatio / Parameter.StressLimit;
       ActualExtract := ActualExtract + Extract[iLayer];
       Soil.MaxExtract := Soil.MaxExtract + Soil.WaterLayer[iLayer].ExtractEffort;
       End
    Else
       Extract[iLayer] := 0;
    End;
If Soil.MaxExtract > 0 then
   ActualExtract := ActualExtract / Soil.MaxExtract
Else
   ActualExtract := 0;
For iLayer := 1 to Soil.nLayers do  // no plant water extraction out of litter layer
...
For iLayer := 1 to Soil.nLayers do
    ...
    If Extract[iLayer] > 0 then
       Soil.WaterLayer[iLayer].WaterContent := Soil.WaterLayer[iLayer].WaterContent - Derived.Transpiration *
                                       Extract[iLayer] / Soil.MaxExtract;
    If Soil.WaterLayer[iLayer].WaterContent < 0 then Soil.WaterLayer[iLayer].WaterContent := 0;
...
Procedure GetWeather;
Var   Pd, DCLN, RelDaylen, Q0, DewPointT: real48;
      i, CountLines: Integer;
      St, DataIn, MessageToUser: String;
      FileFound, StartingDateReached: Boolean;
      WeatherOpts: WeatherFileOptions;
Label ReadNextWeatherRecord;
      ...
      Procedure ReadTheDate (var StartingDateReached: Boolean);
      var LineIn, D1, D2, D3, DD, MM, YY: String;
          CharPos, Days, Months, Years, ErrCode: Integer;

          Procedure GetDate (var Date1, Date2, Date3: String);
          Begin
          Date1 := D1;
          Date2 := D2;
          Date3 := D3;
          ...
          Procedure GetDateNoSeperator (var Date1, Date2, Date3: String; S1, S2, S3: Integer);
          Begin
          Date1 := Copy(LineIn, 1, S1);
          Date2 := Copy(LineIn, S1 + 1, S2);
          Date3 := Copy(LineIn, S1 + S2 + 1, S3);
          End; {of Procedure 'GetDate'}

      Begin
      Read (Control.DefClim, LineIn);
      CharPos := 1;
      While (CharPos = 1) do {Remove leading spaces}
            ...
            End;
      if Control.DateSeperator <> '' then
         Begin
         CharPos := Pos(Control.DateSeperator, LineIn);
         D1 := Copy(LineIn, 1, CharPos - 1);
         LineIn := Copy(LineIn, Charpos + 1, length(LineIn));
         CharPos := Pos(Control.DateSeperator, LineIn);
         D2 := Copy(LineIn, 1, CharPos - 1);
         D3 := Copy(LineIn, Charpos + 1, length(LineIn));
         if Control.DateSystem = 'DDMMYYYY' then
            GetDate (DD, MM, YY)
         Else if Control.DateSystem = 'MMDDYYYY' then
            GetDate (MM, DD, YY)
         Else if Control.DateSystem = 'YYYYMMDD' then
            GetDate (YY, MM, DD)
         ...
         Else
            GetDate (DD, MM, YY);
         End
      Else  // no date seperator
         Begin
         if Control.DateSystem = 'DDMMYYYY' then
            GetDateNoSeperator (DD, MM, YY, 2, 2, 4)
         Else if Control.DateSystem = 'MMDDYYYY' then
            GetDateNoSeperator (MM, DD, YY, 2, 2, 4)
         Else if Control.DateSystem = 'YYYYMMDD' then
            GetDateNoSeperator (YY, MM, DD, 4, 2, 2)
         Else if Control.DateSystem = 'YYYYDDMM' then
            GetDateNoSeperator (YY, DD, MM, 4, 2, 2)
         Else
            GetDateNoSeperator (DD, MM, YY, 2, 2, 4);
         End;
      Val(DD, Days, ErrCode);
      if ErrCode = 0 then
         Control.ExtraDays := Days;
      Val(MM, Months, ErrCode);
      if ErrCode = 0 then
         Control.ExtraMonths := Months;
      Val(YY, Years, ErrCode);
      if ErrCode = 0 then
         Control.TotalYears := Years;
      StartingDateReached := true;
      if Control.UseStartingDate then   // we want to use this date to set the starting date for deciding when to start the proper run. So, it takes more checking
         Begin
         if (380 * Control.TotalYears + 31 * Control.ExtraMonths + Control.ExtraDays) <
            (380 * Control.StartingYear + 31 * Control.StartingMonth + Control.StartingDay) then
            Begin
            StartingDateReached := false;
            Readln (Control.DefClim);
            if eof (Control.DefClim) then  // reached the end of the file and either still no startingdate found or we are back to before the starting date
               Begin
               If eof(Control.Defclim) then
                  ...
                  End;
               MessageDlg(' UNSUITABLE DATES IN CLIMATE FILE!' + chr(10) +
                          'You specified to run the program only for specified' + chr(10) +
                          'dates from the climate file, but unuseable dates have been found.' + chr(10) +
                          'Either, there are no dates within the specified range or.' + chr(10) +
                          'the dates are not read correctly.' + chr(10) +
                          'The run will be continued with the automatic date controlled swicthed off!' + chr(10) +
                          'You can switch it back on under ''Control Paramters''',
                          mtConfirmation, [mbYes, mbNo], 0);
                 Control.UseStartingDate := false;
               End;
            End;
         End;
      End; {of Procedure 'ReadTheDate'}
...
if (Control.ClimType = 'O') and (not Control.ClimFileOpen) then
   ...
   If not FileFound then
      ...
      Else
           Begin
           MessageDlg('PROGRAM RUN WILL BE ABORTED' + chr(10) +
...
Else if (Control.ClimType = 'O') and Control.Run_On then
   Begin
   if not Control.UseStartingDate then
      StartingDateReached := true;
   ReadNextWeatherRecord:
   If (not Control.ClimFileOpen) then
          ...
          If not Control.StartRun then
             ...
             End;
          End;
   For WeatherOpts := W_Tmax to W_Date do
          If WeatherFile[WeatherOpts] then
             Case WeatherOpts of
               ...
               W_Date:  ReadTheDate (StartingDateReached);
               End;
       If not StartingDateReached then
          Goto ReadNextWeatherRecord;
       Readln (Control.DefClim);
       If eof(Control.Defclim) then
          Begin
          CLOSE (Control.DefClim);
   ...
   Weather.LastMin := Weather.LastMin + Weather.ExtraTemp;
   ...
   Else
      ...
      Else if Weather.Tmean > 0 then
         ...
         If (-Weather.Tsoil / Parameter.SoilTResist) <
            ((Weather.Tmean * Derived.SoilTRadnEffect - Weather.Tsoil) / (Parameter.SoilTResist + Soil.Snow * Parameter.SnowInsulate)) then
         ...
         Else
             Weather.Tsoil := Weather.Tsoil + (Weather.Tmean * Derived.SoilTRadnEffect - Weather.Tsoil) /
               (Parameter.SoilTResist + Soil.Snow * Parameter.SnowInsulate);
         End
      Else
         Weather.Tsoil := Weather.Tsoil + (Weather.Tmean * Derived.SoilTRadnEffect - Weather.Tsoil) /
                         (Parameter.SoilTResist + Soil.Snow * Parameter.SnowInsulate);
...
var St: string[String_t_len];
    GrazingAdded: Array[ElementsUsed] of Real48;
    E: ElementsUsed;
    PercentageGrazed, AboveGroundFraction: Real48;
Begin
Derived.GrazerRespiration := 0;
...
If Event.Irrigate then
   ...
   If Event.DaysSinceIrrigation = Event.IrrigationInterval then
      ...
      Else
         ...
         Else
            Begin
            If (Soil.TotalWater / Soil.MaxWater) < Event.IrrigationFraction then
                Derived.IrrigateWater := Event.IrrigationFraction * Soil.MaxWater - Soil.TotalWater;
...
If Control.NextFertilise <= Event.nFertilisations then
   If Event.FertiliseTimes[Control.NextFertilise, 4] <= Control.TotalDays then
      ...
      Event.PFertiliserAdded := Event.PFertiliserAdded + Event.FertiliseAmount[Control.NextFertilise, P];
...
If Control.NextGrazing <= Event.nGrazings then
   If Event.GrazingTimes[Control.NextGrazing, 4] <= Control.TotalDays then
      ...
      Else // if EventGrazingUnits = 'W'
         ...
         End;
      AboveGroundFraction := Divide((Plant.Leaves[C] + Plant.Fruit[C] + Plant.Pollen[C] + Plant.Reserves[C] + Plant.Branches[C]),
                                    (Plant.Leaves[C] + Plant.Fruit[C] + Plant.Pollen[C] + Plant.Reserves[C] + Plant.Branches[C] + Plant.FineRoot[C]));
      for E := C to P do
          Begin
          GrazingAdded[E] := (Plant.Leaves[E] + Plant.Fruit[E] + Plant.Pollen[E] + Plant.Reserves[E] + Plant.Branches[E] +
                              Plant.Soluble[E] * AboveGroundFraction) * PercentageGrazed
                            + Event.GrazingSupplement[Control.NextGrazing, E] / Control.CConversion;
          Plant.Leaves[E] := Plant.Leaves[E] * (1 - PercentageGrazed);
          Plant.Fruit[E] := Plant.Fruit[E] * (1 - PercentageGrazed);
          Plant.Pollen[E] := Plant.Pollen[E] * (1 - PercentageGrazed);
          Plant.Reserves[E] := Plant.Reserves[E] * (1 - PercentageGrazed);
          Plant.Soluble[E] := Plant.Soluble[E] * (1 - PercentageGrazed);
          Plant.Branches[E] := Plant.Branches[E] * (1 - PercentageGrazed);
          Derived.Removed[E] := Derived.Removed[E] + GrazingAdded[E] * (Event.GrazingFractionRemoved[Control.NextGrazing, E]);
          if E = C then
             Litter.OMAddition[E] := Litter.OMAddition[E] + GrazingAdded[E] * (1 - Event.GrazingFractionRespired[Control.NextGrazing] - Event.GrazingMethaneFraction[Control.NextGrazing - 1]
                                  - Event.GrazingFractionRemoved[Control.NextGrazing, E] - Event.Grazing2FractionLeached[Control.NextGrazing, E])
          Else
             Litter.OMAddition[E] := Litter.OMAddition[E] + GrazingAdded[E] *
                                  (1 - Event.GrazingFractionRemoved[Control.NextGrazing, E] - Event.GrazingFractionLeached[Control.NextGrazing, E]);
          End;
      Derived.GrazerRespiration := Derived.GrazerRespiration + GrazingAdded[C] * (Event.GrazingFractionRespired[Control.NextGrazing]);
      Derived.CH4Flux := Derived.CH4Flux + 1.3333 * GrazingAdded[C] * Event.GrazingMethaneFraction[Control.NextGrazing - 1];       // The term 1.3333 = 16/12 accounts for the different molecular weights of CH4 and C. Output is expressed in unit of kgCH4
      Derived.NLeached := Derived.NLeached + (GrazingAdded[N] + Event.GrazingSupplement[Control.NextGrazing, N]) * Event.GrazingFractionLeached[Control.NextGrazing, N];
      Derived.PLeached := Derived.PLeached + (GrazingAdded[P] + Event.GrazingSupplement[Control.NextGrazing, P]) * Event.GrazingFractionLeached[Control.NextGrazing, P];
      Control.NextGrazing := Control.NextGrazing + 1;
      End; {of statement under 'If Control.NextGrazing ..'}
If Control.NextGrazing2 <= Event.nGrazings2 then
   If Event.Grazing2Times[Control.NextGrazing2, 4] <= Control.TotalDays then
      Begin
      Control.GrazingFormulaOn := true;
      Control.NextGrazing2 := Control.NextGrazing2 + 1;
      End;
if Control.GrazingFormulaOn then
   if (Event.Grazing2UpLimit[Control.NextGrazing2 - 1] / Control.CConversion) < Plant.Leaves[C] then
      Begin
      St := 'Grazing occurred on ' + Control.Date;
      AddNotice(St);
      GrazingAdded[C] := (Event.Grazing2UpLimit[Control.NextGrazing2 - 1] - Event.Grazing2LowLimit[Control.NextGrazing2 - 1]) / Control.CConversion;
      if GrazingAdded[C] > (0.99 * Plant.Leaves[C]) then
         GrazingAdded[C] := 0.99 * Plant.Leaves[C]; // set a maximum of removing 99% of foliage
      PercentageGrazed := Divide(GrazingAdded[C], Plant.Leaves[C]);
      AboveGroundFraction := Divide((Plant.Leaves[C] + Plant.Fruit[C] + Plant.Pollen[C] + Plant.Reserves[C] + Plant.Branches[C]),
      ...
      for E := C to P do
          Begin
          GrazingAdded[E] := (Plant.Leaves[E] + Plant.Fruit[E] + Plant.Pollen[E] + Plant.Reserves[E] + Plant.Branches[E] + Plant.Soluble[E] * AboveGroundFraction) * PercentageGrazed;
          Plant.Leaves[E] := Plant.Leaves[E] * (1 - PercentageGrazed);
          Plant.Fruit[E] := Plant.Fruit[E] * (1 - PercentageGrazed);
          Plant.Pollen[E] := Plant.Pollen[E] * (1 - PercentageGrazed);
          Plant.Reserves[E] := Plant.Reserves[E] * (1 - PercentageGrazed);
          Plant.Soluble[E] := Plant.Soluble[E] * (1 - PercentageGrazed);
          Plant.Branches[E] := Plant.Branches[E] * (1 - PercentageGrazed);
          Derived.Removed[E] := Derived.Removed[E] + GrazingAdded[E] * (Event.Grazing2FractionRemoved[Control.NextGrazing2 - 1, E]);
          if E = C then
             Litter.OMAddition[E] := Litter.OMAddition[E] + GrazingAdded[E] * (1 - Event.Grazing2FractionRespired[Control.NextGrazing2 - 1] - Event.Grazing2MethaneFraction[Control.NextGrazing2 - 1]
                                  - Event.Grazing2FractionRemoved[Control.NextGrazing2 - 1, E] - Event.Grazing2FractionLeached[Control.NextGrazing2 - 1, E])
          Else
             Litter.OMAddition[E] := Litter.OMAddition[E] + GrazingAdded[E] *
                                  (1 - Event.Grazing2FractionRemoved[Control.NextGrazing2 - 1, E] - Event.Grazing2FractionLeached[Control.NextGrazing2 - 1, E]);
          End;
      Derived.GrazerRespiration := Derived.GrazerRespiration + GrazingAdded[C] * (Event.Grazing2FractionRespired[Control.NextGrazing2 - 1]);
      Derived.CH4Flux := Derived.CH4Flux + 1.3333 * GrazingAdded[C] * Event.Grazing2MethaneFraction[Control.NextGrazing2 - 1];    // The term 1.3333 = 16/12 accounts for the different molecular weights of CH4 and C. Output is expressed in unit of kgCH4
      Derived.NLeached := Derived.NLeached + GrazingAdded[N] * Event.Grazing2FractionLeached[Control.NextGrazing2 - 1, N];
      Derived.PLeached := Derived.PLeached + GrazingAdded[P] * Event.Grazing2FractionLeached[Control.NextGrazing2 - 1, P];
      End; {of statement under 'Control.GrazingFormulaOn ..'}
If Control.NextSeeding <= Event.nSeedings then
   If Event.SeedingTimes[Control.NextSeeding, 4] <= Control.TotalDays then
      Begin
      if (Plant.Sapwood[C] + Plant.HeartWood[C]) > 100 then
      // Warning: this currently does not adjust average tree dimensions. It is intended only for seedings when there are minimal existing stocking and plant biomass
      // A warning is added to the notice page
         St := 'Extra seeds added on ' + Control.Date + ' but average stand dimensions have not been adjusted. This will lead to anomalous forest growth if applied to sizeable existign stands!!'
      Else
         St := 'Extra seeds added on ' + Control.Date;
      AddNotice(St);
      Plant.Stocking := Plant.Stocking + Event.PlantsAdded[Control.NextSeeding];
      Plant.Reserves[C] := Plant.Reserves[C] + Event.BiomassAdded[Control.NextSeeding, C];
      Plant.Reserves[N] := Plant.Reserves[N] + Event.BiomassAdded[Control.NextSeeding, N];
      Plant.Reserves[P] := Plant.Reserves[P] + Event.BiomassAdded[Control.NextSeeding, P];
      Control.NextSeeding := Control.NextSeeding + 1;
      End;
