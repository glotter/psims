...
type
  TfrmMain = class(TForm)
    ...
    B1: TMenuItem;
    mnuGrazing: TMenuItem;
    IntroImage: TImage;
    Nitrogenpools1: TMenuItem;
    actnIncludePhosphorus: TAction;
    mnuPlantNitrogen: TMenuItem;
    actIncludePDisplay: TAction;
    mnuSavephosphorusInfo: TMenuItem;
    actIncludePSave: TAction;
    mnuEquilibrium: TMenuItem;
    Ploughing1: TMenuItem;
    mnuWeedParameters: TMenuItem;
    N1: TMenuItem;
    OMAdditions1: TMenuItem;
    SensitivityTest: TMenuItem;
    Grazing_Formula: TMenuItem;
    Seeding: TMenuItem;
    ...
    Procedure mnuPhosphorusPoolsClick(Sender: TObject);
    ...
    Procedure mnuGrazingClick(Sender: TObject);
    Procedure mnuPlantcarbonClick(Sender: TObject);
    Procedure mnuPhosphorusClick(Sender: TObject);
    ...
    Procedure mnuSpatialClick(Sender: TObject);
    ...
    Procedure actnIncludePhosphorusExecute(Sender: TObject);
    Procedure actIncludeDeltaExecute(Sender: TObject);
    Procedure actIncludeWeedParametersExecute(Sender: TObject);
    ...
    Procedure NitrogenpoolsClick(Sender: TObject);
    Procedure mnuPlantNitrogenClick(Sender: TObject);
    Procedure actIncludePDisplayExecute(Sender: TObject);
    Procedure mnuSavephosphorusInfoClick(Sender: TObject);
    Procedure actIncludePSaveExecute(Sender: TObject);
    procedure mnuEquilibriumClick(Sender: TObject);
    procedure PloughingClick(Sender: TObject);
    procedure mnuWeedParametersClick(Sender: TObject);
    procedure mnuSensitivityClick(Sender: TObject);
    procedure OMAdditionsClick(Sender: TObject);
    procedure mnuParameterFittingClick(Sender: TObject);
    procedure Grazing_FormulaClick(Sender: TObject);
    procedure SeedingClick(Sender: TObject);
...
uses
  ...
  untFittingParameters, untProcessFittingData, untFitSelect;
...
Procedure SetInitialProjectDefaults;
    ...
    Event.nGrazings := 0;
    Event.nSeedings := 0;
    Event.PloughDisturbanceRecovery := 0.9836065574; // Time constant of 2 months
...
Procedure SetInitialPlantDefaults;
    ...
    Parameter.Three_Two_Power_Law_Slope := 3/2;
    Parameter.RootDesicSensitivity := 1;
...
Procedure SetInitialSiteDefaults;
    ...
    For iLayer := 1 to MaxSoilLayers do
        Soil.WaterLayer[iLayer].StressSensitivity := 0;
...
procedure TfrmMain.SeedingClick(Sender: TObject);
Begin
...
Procedure TfrmMain.Grazing_FormulaClick(Sender: TObject);
Begin
Repeat
...
Control.RunType := Standard;
  ...
  try
    If (Control.AgreeOK) then
      ...
      Derived.ExcessN := 0;
      frmFileIO.GetParameterFile(Control.PoolFile, EXT_INITIAL);
      if NOT Control.SoilLayerError then
         Begin
...
Begin
Control.RunType := BatchRun;
OldScreen[D_Batch].First := true;
Control.Parfit.FittingMode := false;
InitialBatch;
   ...
   Procedure AdjustPPools (var LastPool, NewPool: Real48; Delta: Real48; Direction: Char);
    ...
    For E := C to P do
        For iLayer := 0 to nLayers do
            Soil.Inert[iLayer, E] := 0;
    if Control.IncludeP then
        For iLayer := 0 to nLayers do
            Soil.RockP[iLayer, P] := 0;
...
If Control.AgreeOK and Control.EquilMode then
   ...
   InitialData.Soil := Soil;
   InitialData.Inert := Soil.Inert;
   InitialData.RockP := Soil.RockP;
   ...
   For E := C to LastE do
     For iLayer := 0 to nLayers do
       Begin
       CheckLimits(Soil.Slow[iLayer, E], 0 , 1e6);
       CheckLimits(Soil.Resistant[iLayer, E], 0 , 1e6);
       CheckLimits(Soil.Active[iLayer, E], 0 , 1e6);
       CheckLimits(Soil.Struct[iLayer, E], 0 , 1e6);
       CheckLimits(Soil.Metab[iLayer, E], 0 , 1e6);
       CheckLimits(Soil.FineWood[iLayer, E], 0 , 1e6);
       CheckLimits(Soil.CoarseWood[iLayer, E], 0 , 1e6);
       end;
   StartStruct := Soil.Struct;
   StartFineWood := Soil.FineWood;
   StartCoarseWood := Soil.CoarseWood;
   StartActive := Soil.Active;
   StartSlow := Soil.Slow;
   StartResistant := Soil.Resistant;
   // Start of Sarch Loop
   If (Control.Equil.MaxIterations > 0) and (not Abort) then
      Repeat
        ...
        If Control.Equil.SamePlantPools then
           ...
           Soil := InitialData.Soil;
           End;
        LastStruct := Soil.Struct;
        LastFineWood := Soil.FineWood;
        LastCoarseWood := Soil.CoarseWood;
        LastActive := Soil.Active;
        LastSlow := Soil.Slow;
        LastResistant := Soil.Resistant;
{        Soil.Struct := StartStruct;
        Soil.FineWood := StartFineWood;
        Soil.CoarseWood := StartCoarseWood;
        Soil.Active := StartActive;
        Soil.Slow := StartSlow;
        Soil.Resistant := StartResistant;}
        OldAllN := TotalN;
        IF (Control.ClimType = 'S') or
           ((Control.ClimType = 'O') and not WeatherFile[W_Rain]) THEN
           RandSeed := 0;  // to ensure that the random sequence is made 'non-random' for this procedure here
        ControlRun;
        ...
        For iLayer := 0 to nLayers do
            Begin
            Derived.Equil.Converg2 := Derived.Equil.Converg2 + abs(Divide((Soil.Slow[iLayer, N] - LastSlow[iLayer, N]), Soil.Slow[iLayer, N]));
            Derived.Equil.Converg3 := Derived.Equil.Converg3 + abs(Divide((Soil.Resistant[iLayer, N] - LastResistant[iLayer, N]), Soil.Resistant[iLayer, N]));
            End;
        ...
        if Control.IncludeP then
           ...
           Else
              for iLayer := 0 to nLayers do
                  Begin
                  AdjustPPools (LastCoarseWood[iLayer, P], Soil.CoarseWood[iLayer, P], PAdjust, Direction);
                  AdjustPPools (LastFineWood[iLayer, P], Soil.FineWood[iLayer, P], PAdjust, Direction);
                  AdjustPPools (LastStruct[iLayer, P], Soil.Struct[iLayer, P], PAdjust, Direction);
                  AdjustPPools (LastActive[iLayer, P], Soil.Active[iLayer, P], PAdjust, Direction);
                  AdjustPPools (LastSlow[iLayer, P], Soil.Slow[iLayer, P], PAdjust, Direction);
                  AdjustPPools (LastResistant[iLayer, P], Soil.Resistant[iLayer, P], PAdjust, Direction);
                  End;
           End;
        ...
        If Derived.Equil.ManualAdjust then
           ...
           For E := C to LastE do
             For iLayer := 0 to nLayers do
                Begin
                LastCoarseWood[iLayer, E] := Soil.CoarseWood[iLayer, E];
                LastFineWood[iLayer, E] := Soil.FineWood[iLayer, E];
                LastStruct[iLayer, E] := Soil.Struct[iLayer, E];
                LastActive[iLayer, E] := Soil.Active[iLayer, E];
                LastSlow[iLayer, E] := Soil.Slow[iLayer, E];
                LastResistant[iLayer, E] := Soil.Resistant[iLayer, E];
                End;
           Derived.Equil.ManualAdjust := false;
           End
        Else
          For E := C to LastE do
             For iLayer := 0 to nLayers do
                Begin
                SignFlip(Soil.CoarseWood[iLayer, E], LastCoarseWood[iLayer, E], FlipCoarseWood[iLayer, E], SignCoarseWood[iLayer, E]);
                SignFlip(Soil.FineWood[iLayer, E], LastFineWood[iLayer, E], FlipFineWood[iLayer, E], SignFineWood[iLayer, E]);
                SignFlip(Soil.Struct[iLayer, E], LastStruct[iLayer, E], FlipStruct[iLayer, E], SignStruct[iLayer, E]);
                SignFlip(Soil.Active[iLayer, E], LastActive[iLayer, E], FlipActive[iLayer, E], SignActive[iLayer, E]);
                SignFlip(Soil.Slow[iLayer, E], LastSlow[iLayer, E], FlipSlow[iLayer, E], SignSlow[iLayer, E]);
                SignFlip(Soil.Resistant[iLayer, E], LastResistant[iLayer, E], FlipResistant[iLayer, E], SignResistant[iLayer, E]);
                if Derived.Equil.Iterations <> 0 then
                   Begin
                   AdjustSOMPools (LastCoarseWood[iLayer, E], Soil.CoarseWood[iLayer, E], StartCoarseWood[iLayer, E] {StartSOM was an attempt to separately record starting SOM}, FlipCoarseWood[iLayer, E], SOMPoolAdjustment);
                   AdjustSOMPools (LastFineWood[iLayer, E], Soil.FineWood[iLayer, E], StartFineWood[iLayer, E], FlipFineWood[iLayer, E], SOMPoolAdjustment);
                   AdjustSOMPools (LastStruct[iLayer, E], Soil.Struct[iLayer, E], StartStruct[iLayer, E], FlipStruct[iLayer, E], SOMPoolAdjustment);
                   AdjustSOMPools (LastActive[iLayer, E], Soil.Active[iLayer, E], StartActive[iLayer, E], FlipActive[iLayer, E], SOMPoolAdjustment);
                   AdjustSOMPools (LastSlow[iLayer, E], Soil.Slow[iLayer, E], StartSlow[iLayer, E], FlipSlow[iLayer, E], SOMPoolAdjustment);
                   AdjustSOMPools (LastResistant[iLayer, E], Soil.Resistant[iLayer, E], StartResistant[iLayer, E], FlipResistant[iLayer, E], SOMPoolAdjustment * Control.Equil.BoostResistant);
...
{   Soil.Struct := StartStruct;
   ...
   Plant.dbh := InitialData.dbh;
   Plant.Age := InitialData.Age;
   Soil.Inert := InitialData.Inert;
   Soil.RockP := InitialData.RockP;
   If not Abort then
...
Procedure TfrmMain.mnuSensitivityClick(Sender: TObject);
var i: Integer;
Begin
If Control.AgreeOK then
   Begin
...
procedure TfrmMain.mnuParameterFittingClick(Sender: TObject);
var GetOut: Boolean;
    SetFitVar: FittingType;
    i, j: Integer;
    delta1, delta2: Real48;
...
Begin
FitParameter.SensMultiplier := 1;
Control.TSoilFound := false;
Obs.RunBatch := Control.BatchMode;
Control.BatchMode := false;
Control.RunType := ParFit;
OldScreen[D_ParFit].First := true;
Control.Parfit.EndFit := false;
Control.Parfit.FittingMode := true;
If Control.AgreeOK then
   Begin
   for SetFitVar := Succ(F_Dummy) to Pred(F_EndDummy) do
       Begin
       FitParameter.deltaX[SetFitVar] := Control.ParFit.Delta;
       FitParameter.Varied[SetFitVar] := 0;
       FitParameter.Initial[SetFitVar] := 0;
       FitParameter.Best[SetFitVar] := 0;
       FitParameter.Last[SetFitVar] := 0;
       End;
   FitParameter.Index := 1;
   FitParameter.Abort := false;
   frmFitSelect.ShowModal;
   SetUpFitParameters(1);
   FitParameter.Index := 2;
   frmFitSelect.ShowModal;
   SetUpFitParameters(2);
   if not FitParameter.Abort then
      Begin
      If frmFittingParameters.ShowModal = mrOK then
         FitParameter.Abort := false
      Else
   ...
   IF (Control.ClimType = 'S') or
     ((Control.ClimType = 'O') and (not WeatherFile[W_Rain])) THEN
      RandSeed := 0;  // to ensure that the random sequence is made 'non-random' for this procedure here
   FitParameter.OldSumOfSquares := 1e9;
   FitParameter.Best := FitParameter.Varied;
   If (Control.ParFit.MaxIterations > 0) and (not FitParameter.Abort) then
      Begin
      ...
      End;
   frmEquilProgress.Stop;
   SetUpFitParameters (0);
//   if not Control.Abort then  - need to change the normal interrupt to the abort used here; then can implement this
      Begin
      FitParameter.Index := 3;
      frmFitSelect.ShowModal;
      End;
   SetUpFitParameters(-1);
   Control.PlantHasChanged := true;  // need to be more specific about plant or site
   Control.SiteHasChanged := true;   // need to be more specific about plant or site
...
Else if not Control.AgreeOK then
   MessageDlg('CANNOT RUN SIMULATION' + chr(10) +
                 ...
                 'your acceptance of the licence agreement.',
                 mtError, [mbOK], 0);
end; {of Procedure 'mnuParameterFitting'}
...
Begin
Control.Parfit.FittingMode := false;
Control.RunType := Geographic;
for iLayer := 1 to Soil.nLayers do
    OldWaterHold := OldwaterHold + Soil.WaterLayer[iLayer].WaterContent;
...
If Control.AgreeOK and Control.SpatialMode then
      ...
      While (not Control.EndSpatial) and
            ...
            Soil.WaterLayer[1].MaxWater := 10;
            Soil.WaterLayer[2].MaxWater := Control.Spatial.WaterHold - Soil.WaterLayer[1].MaxWater;
            Soil.WaterLayer[1].WaterContent := 0.5 * Soil.WaterLayer[1].MaxWater;
            Soil.WaterLayer[2].WaterContent := 0.5 * Soil.WaterLayer[2].MaxWater;
            ...
            If not Control.EndSpatial then
               ...
               If (Control.Spatial.Count = 1) or
                  ...
                  If Control.Spatial.SoilType = Equil then
                     ...
                     If SlowC <> 0 then
                        Begin
                        Soil.Active[1, C] := Control.Spatial.ActiveC;
                        Soil.Active[1, N] := Control.Spatial.ActiveN;
                        Soil.Slow[1, C] := Control.Spatial.SlowC;
                        Soil.Slow[1, N] := Control.Spatial.SlowN;
                        Soil.Resistant[1, C] := Control.Spatial.ResistC;
                        Soil.Resistant[1, N] := Control.Spatial.ResistN;
                        Soil.Struct[0, C] := Soil.Struct[0, C] * TargetN / Parameter.Ncrit;
                        Soil.Struct[0, N] := Soil.Struct[0, N] * sqr(TargetN / Parameter.Ncrit);
                        Soil.Struct[1, C] := Soil.Struct[1, C] * TargetN / Parameter.Ncrit;
                        Soil.Struct[1, N] := Soil.Struct[1, N] * sqr(TargetN / Parameter.Ncrit);
                        Soil.Metab[0, N] := Soil.Metab[0, C] * TargetN / Parameter.Ncrit;
                        Soil.Metab[0, N] := Soil.Metab[0, N] * sqr(TargetN / Parameter.Ncrit);
                        Soil.Metab[1, C] := Soil.Metab[1, C] * TargetN / Parameter.Ncrit;
                        Soil.Metab[1, N] := Soil.Metab[1, N] * sqr(TargetN / Parameter.Ncrit);
                        Plant.Stocking := 1000;
                        Plant.Height := 1;
                        Plant.DBH := 1;
                        Plant.SapWood[C] := 0;
                     ...
                     Else
                        frmFileIO.GetParameterFile(Control.PoolFile, 'IL!');
                     OldSlow := Soil.Slow[1, N] + Soil.Resistant[1, N] + Soil.Active[1, N];
                     Deltax := 1;
                     DeltaY := 0.1;
                     TargetHit := 0;
                     End
                  ...
                  If Control.Spatial.SoilType = Equil then
                     Begin
                     SlowC := Soil.Slow[1, C] * Parameter.Ncrit / TargetN;
                     SlowN := Soil.Slow[1, N] * Parameter.Ncrit / TargetN;
                     ActiveC:= Soil.Active[1, C] * Parameter.Ncrit / TargetN;
                     ActiveN:= Soil.Active[1, N] * Parameter.Ncrit / TargetN;
                     ResistantC := Soil.Resistant[1, C] * Parameter.Ncrit / TargetN;
                     ResistantN := Soil.Resistant[1, N] * Parameter.Ncrit / TargetN;
                     Soil.Struct[0, C] := Soil.Struct[0, C] * Parameter.Ncrit / TargetN;
                     Soil.Struct[0, N] := Soil.Struct[0, N] * sqr(Parameter.Ncrit / TargetN);
                     Soil.Struct[1, C] := Soil.Struct[1, C] * Parameter.Ncrit / TargetN;
                     Soil.Struct[1, N] := Soil.Struct[1, N] * sqr(Parameter.Ncrit / TargetN);
                     Soil.Metab[0, N] := Soil.Metab[0, C] * Parameter.Ncrit / TargetN;
                     Soil.Metab[0, N] := Soil.Metab[0, N] * sqr(Parameter.Ncrit / TargetN);
                     Soil.Metab[1, C] := Soil.Metab[1, C] * Parameter.Ncrit / TargetN;
                     Soil.Metab[1, N] := Soil.Metab[1, N] * sqr(Parameter.Ncrit / TargetN);
...
//                  PrintOut(msg);
               ...
               If Control.Spatial.ShowNumeric and not OutOfBounds then
                  ...
                  Writeln (Control.CenWFileOut, Control.Spatial.Count:5, RainSum:5:0, TempSum:6:1,
                       ...
                       Soil.Slow[1, C]:7:0, (Plant.SapWood[C] + Plant.HeartWood[C]):7:0,
      ...
      Soil.WaterLayer[1].MaxWater := 20;
      Soil.WaterLayer[2].MaxWater := OldWaterHold - Soil.WaterLayer[1].MaxWater;
