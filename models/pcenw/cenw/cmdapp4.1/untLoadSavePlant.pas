...
ProcessNext (Parameter.Albedo, 'Leaf albedo', 0.01, CommentStr, VarStr, ReadWrite, Defp);
...
ProcessNext (Parameter.RootDesicSensitivity, 'Relative sensitivity of root death (compared to that of leaves) during drought', 1, CommentStr, VarStr, ReadWrite, Defp);
...
ProcessNext (Parameter.Transmit, 'Leaf transmissivity', 0.01, CommentStr, VarStr, ReadWrite, Defp);
...
ProcessNext (Parameter.Old_3_2_Power_Law, 'Parameter in the 3/2 power law relationship', exp(15.5), CommentStr, VarStr, ReadWrite, Defp);
ProcessNext (Parameter.Three_Two_Power_Law, 'Parameter in the 3/2 power law relationship (saved in the log form)', 15.5, CommentStr, VarStr, ReadWrite, Defp);
ProcessNext (Parameter.Three_Two_Power_Law_Slope, 'Slope of the 3/2 power law relationship (in case it is not 3/2)', 1.5, CommentStr, VarStr, ReadWrite, Defp);
...
ProcessNext (Parameter.MatureAge, 'Age of maturity (at which NPP is only 50% of young-tree NPP', 200, CommentStr, VarStr, ReadWrite, Defp);
ProcessNext (Parameter.MatureSize, 'Size of maturity (at which NPP is only 50% of young-tree NPP', 100, CommentStr, VarStr, ReadWrite, Defp);
...
Assign (Defp, Name);
Reset (Defp);
Parameter.Old_3_2_Power_Law := 0;
Parameter.Three_Two_Power_Law := 0; // Shifting over to using the log form of the parameter - but need to see whether an old file is read with the exponential form
...
if Parameter.WoodDensity <> 0 then // we are loading information from an old file
   ...
   End;
If Parameter.Three_Two_Power_Law = 0 then   // haven't read any numbers from the log format, yet
   if Parameter.Old_3_2_Power_Law > 0 then
      Parameter.Three_Two_Power_Law := ln(Parameter.Old_3_2_Power_Law)
   Else
      Parameter.Three_Two_Power_Law := 15.5;     // a default value
...
Parameter.Old_3_2_Power_Law := 0;      // to save a dummy zero for this superceded exponential form of the parameter
