...
var fp : Text;
    ...
    ErrCode, iLayer, i, RunLayers, SoilnLayers: integer;
    E: ElementsUsed;
        ...
        Procedure TSoilIn (Var Item: double; Comment: String);
...
for E := C to P do
    Begin
    Litter.WeedRoots[E] := 0;  // In case no values had previously been saved
    Litter.WeedLeaves[E] := 0;  // In case no values had previously been saved
    End;
Control.CountersSaved := false;   // remains false unless it finds the relevant data in 'GetCounters'
For iLayer := 0 to MaxSoilLayers do
    Derived.Disturbed[iLayer] := 1;    // Set to 1 in case no values have been saved yet (from an earlier version)
while not eof(fp) do
      Begin
      Readln(fp, Linein);
      ProcessLine (Linein, VarStr, CommentStr);
      IntegerIn (SoilnLayers, 'Number of layers for soil organic matter');
      if SoilnLayers = Soil.nLayers then
         Control.SoilLayerError := false
      Else
         Begin
         Control.SoilLayerError := true;
         End;
      SoilElementIn (Soil.Struct, 'in structural litter');
      SoilElementIn (Soil.Metab, 'in metabolic litter');
      SoilElementIn (Soil.FineWood, 'in fine wood litter');
      SoilElementIn (Soil.CoarseWood, 'in coarse wood litter');
      SoilElementIn (Soil.Active, 'in active organic matter');
      SoilElementIn (Soil.slow, 'in slow organic matter');
      SoilElementIn (Soil.Resistant, 'in resistant organic matter');
      SoilElementIn (Soil.Inert, 'in inert organic matter');
      SoilElementIn (Soil.Soluble, 'in soluble organic matter');
      SoilElementIn (Soil.RockP, '(in rocks) in');
      SoilElementIn (Soil.OccludedP, '(occluded) in');
      SoilElementIn (Soil.SecondaryInorganicP, '(secondary inorganic) in');
      OldElementIn (Soil.Struct, 'in surface structural litter', 0);
      OldElementIn (Soil.Metab, 'in surface metabolic litter', 0);
      OldElementIn (Soil.Struct, 'in soil structural litter', 1);
      OldElementIn (Soil.Metab, 'in soil metabolic litter', 1);
      OldElementIn (Soil.FineWood, 'in surface fine wood litter', 0);
      OldElementIn (Soil.CoarseWood, 'in surface coarse wood litter', 0);
      OldElementIn (Soil.CoarseWood, 'in soil coarse wood litter', 1);
      ...
      ElementIn (Litter.WeedRoots, 'in dead weed roots');
      ElementIn (Litter.WeedLeaves, 'in dead weed leaves');
      ...
      RealIn (Soil.LitterLig[0], 'Lignin concentration in surface litter');   {Retained to read info from old files}
      RealIn (Soil.LitterLig[1], 'Lignin concentration in soil litter');      {Retained to read info from old files}
      RealIn (Soil.BranchLig[0],  'Lignin concentration in branch litter'); {Retained to read info from old files}
      RealIn (Soil.StemLig[0], 'Lignin concentration in stem litter');      {Retained to read info from old files}
      For iLayer := 0 to Soil.nLayers do
          Begin
          Str (iLayer:3, LayerInfo);
          RealIn (Soil.LitterLig[iLayer], 'Litter lignin concentration in layer' + LayerInfo);
          RealIn (Soil.BranchLig[iLayer],  'Fine-wood lignin concentration in layer' + LayerInfo);
          RealIn (Soil.StemLig[iLayer], 'Coarse-wood lignin concentration in layer' + LayerInfo);
          RealIn (Derived.Disturbed[iLayer], 'Disturbance related stimulation of decomposition' + LayerInfo);
          End;
      RealIn (Soil.Snow, 'Size of snow pack (mm)');
      RealIn (Soil.TotalWater,  'Total water held in the soil');
      RealIn (Soil.MaxExtract, 'Total relative water extraction term for all layers');
      For iLayer := 0 to Soil.nLayers do
          Begin
          Str (iLayer:3, LayerInfo);
          RealIn (Soil.WaterLayer[iLayer].WaterContent, 'Amount of water (mm) in layer' + LayerInfo);
          End;
...
Close(fp);
iLayer := iLayer - 1;
If Soil.nLayers <> iLayer then  {This If-statement is invoked if the number of layers saved
                                     in the parameter file are different from those in the initial pool size file}
   Begin
   Soil.TotalWater := 0;
   For i := 1 to iLayer do
       Soil.TotalWater := Soil.TotalWater + Soil.WaterLayer[i].WaterContent;
   For i := 1 to Soil.nLayers do
       Begin
       If Soil.TotalWater > Soil.WaterLayer[i].MaxWater then
          Begin
          Soil.WaterLayer[i].WaterContent := Soil.WaterLayer[i].MaxWater;
          Soil.TotalWater := Soil.TotalWater - Soil.WaterLayer[i].MaxWater;
          End
       Else
          Begin
          Soil.WaterLayer[i].WaterContent := Soil.TotalWater;
          Soil.TotalWater := 0;
          End;
       End;
   Soil.TotalWater := 0;
   For i := 1 to Soil.nLayers do
       Soil.TotalWater := Soil.TotalWater + Soil.WaterLayer[i].WaterContent;
   End;                 {of 'If Soiler.nLayers <> iLayer then' statement}

If Control.AllOneLayer then
   Begin
   RunLayers := 1;
   If Soil.nLayers <> RunLayers then  {This If-statement is invoked if the number of layers saved
                                     in the parameter file are different from those in the initial pool size file}
      Begin
      SoilElementSum (Soil.Struct);
      SoilElementSum (Soil.Metab);
      SoilElementSum (Soil.FineWood);
      SoilElementSum (Soil.CoarseWood);
      SoilElementSum (Soil.Active);
      SoilElementSum (Soil.Slow);
      SoilElementSum (Soil.Resistant);
      SoilElementSum (Soil.Soluble);
      End;                 {of 'If Soil.nLayers <> RunLayers then' statement}
   Soil.nLayers := RunLayers;
   End;
...
IntegerOut (Soil.nLayers, ' Number of layers for soil organic matter');
for Elements := C13 to P do
        ...
        For iLayer := 0 to Soil.nLayers do
            Begin
            Str (iLayer:3, LayerInfo);
            LineOut (Soil.Struct[iLayer, Elements], ElementInfo + 'in structural litter layer' + LayerInfo);
            LineOut (Soil.Metab[iLayer, Elements], ElementInfo + 'in metabolic litter layer' + LayerInfo);
            LineOut (Soil.FineWood[iLayer, Elements], ElementInfo + 'in fine wood litter layer' + LayerInfo);
            LineOut (Soil.CoarseWood[iLayer, Elements], ElementInfo + 'in coarse wood litter layer' + LayerInfo);
            LineOut (Soil.Active[iLayer, Elements], ElementInfo + 'in active organic matter layer' + LayerInfo);
            LineOut (Soil.Slow[iLayer, Elements], ElementInfo + 'in slow organic matter layer' + LayerInfo);
            LineOut (Soil.Resistant[iLayer, Elements], ElementInfo + 'in resistant organic matter layer' + LayerInfo);
            LineOut (Soil.Inert[iLayer, Elements], ElementInfo + 'in inert organic matter layer' + LayerInfo);
            LineOut (Soil.Soluble[iLayer, Elements], ElementInfo + 'in soluble organic matter layer' + LayerInfo);
            if Elements = P then
               Begin
               LineOut (Soil.RockP[iLayer, Elements], ' Phosphorus (in rocks) in layer' + LayerInfo);
               LineOut (Soil.OccludedP[iLayer, Elements], ' Phosphorus (occluded) in layer' + LayerInfo);
               LineOut (Soil.SecondaryInorganicP[iLayer, Elements], ' Phosphorus (secondary inorganic) in layer' + LayerInfo);
               End;
            End;
        ...
        LineOut (Litter.WeedRoots[Elements], ElementInfo + 'in dead weed roots');
        LineOut (Litter.WeedLeaves[Elements], ElementInfo + 'in dead weed leaves');
...
For iLayer := 0 to Soil.nLayers do
       Begin
       Str (iLayer:3, LayerInfo);
       LineOut (Soil.LitterLig[iLayer], ' Litter lignin concentration in layer' + LayerInfo);
       LineOut (Soil.BranchLig[iLayer],  ' Fine-wood lignin concentration in layer' + LayerInfo);
       LineOut (Soil.StemLig[iLayer], ' Coarse-wood lignin concentration in layer' + LayerInfo);
       LineOut (Derived.Disturbed[iLayer], ' Disturbance related stimulation of decomposition' + LayerInfo);
       End;
LineOut (Soil.Snow,  ' Size of snow pack (mm)');
LineOut (Soil.TotalWater,  ' Total water held in the soil');
LineOut (Soil.MaxExtract, ' Total relative water extraction term for all layers');
IntegerOut (Soil.nLayers, ' Number of layers for soil water');
For iLayer := 0 to Soil.nLayers do
       Begin
       Str (iLayer:3, LayerInfo);
       LineOut(Soil.WaterLayer[iLayer].WaterContent, ' Amount of water (mm) in layer' + LayerInfo);
       End;
