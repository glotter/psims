...
For iLine := 1 to 4 do
    ...
    If SaveVar.Choose[S_CH4Flux] then AllHeadings(iLine, 'CH4 Flux', 'CH4', 'kg/ha/d', '');
    If SaveVar.Choose[S_RemovedC] then AllHeadings(iLine, 'C Removed', 'DW', 'kg/ha/d', '');
    If SaveVar.Choose[S_RemovedN] then AllHeadings(iLine, 'N Removed', 'Nitro', 'kg/ha/d', '');
    ...
    If SaveVar.Choose[S_StoredWater] then AllLayerHeadings(iLine, 'Soil', '', 'mm');
    If Control.IncludeP then
       ...
       If SaveVar.Choose[S_RemovedP] then AllHeadings(iLine, 'P Removed', 'Phosphorus', 'kg/ha/d', '');
...
Procedure SaveRegular;
Type MaterialType = array[ElementsUsed] of double;
var iLayer: integer;
    Sum: double;
    LayerSum: SoilElements;
    SumNecro: MaterialType;

    Procedure DatumOut (Datum, Multiple: double);
    ...
    for Elements := C to P do
        ...
        for iLayer := 0 to Soil.nLayers do
            ...
            Soil.FineWood[iLayer, Elements] +
            Soil.CoarseWood[iLayer, Elements] +
            Soil.Struct[iLayer, Elements] +
            Soil.Metab[iLayer, Elements] +
            Soil.Active[iLayer, Elements] +
            Soil.Slow[iLayer, Elements] +
            Soil.Resistant[iLayer, Elements] +
            Soil.Inert[iLayer, Elements] +
            Soil.Soluble[iLayer, Elements];
        End;
    End; {of Procedure 'GetNecroSum'}

    Procedure LayerOut (Data: SoilElements; E: ElementsUsed; WithLayer: Boolean; Multiple: double);
    var iLayer: Integer;
        Sum: double;
    Begin
    If WithLayer then
...
Begin
      ...
      If SaveVar.Choose[S_CH4Flux] then DatumOut(Derived.CH4Flux, 1);
      If SaveVar.Choose[S_RemovedC] then DatumOut(Derived.Removed[C], Control.CConversion);
      If SaveVar.Choose[S_RemovedN] then DatumOut(Derived.Removed[N], Control.NConversion);
      If SaveVar.Choose[S_TDamage] then
                                   Begin
      ...
      If SaveVar.Choose[S_Snow] then DatumOut(Soil.Snow, 1);
      If SaveVar.Choose[S_Drainage] then DatumOut(Derived.Drainage[Soil.nLayers], 1);
      ...
      If SaveVar.Choose[S_NecroN] then
         ...
         End;
      If SaveVar.Choose[S_CMetabolic] then LayerOut(Soil.Metab, C, Control.OutputByLayer, 0.001);
      If SaveVar.Choose[S_NMetabolic] then LayerOut(Soil.Metab, N, Control.OutputByLayer, 1);
      If SaveVar.Choose[S_CStructural] then LayerOut(Soil.Struct, C, Control.OutputByLayer, 0.001);
      If SaveVar.Choose[S_NStructural] then LayerOut(Soil.Struct, N, Control.OutputByLayer, 1);
      If SaveVar.Choose[S_CWoodyLitter] then
         Begin
         For iLayer := 0 to Soil.nLayers do
             LayerSum[iLayer, C] := Soil.FineWood[iLayer, C] +
                                 Soil.CoarseWood[iLayer, C];
         LayerOut(LayerSum, C, Control.OutputByLayer, 0.001);
         End;
      If SaveVar.Choose[S_NWoodyLitter] then
         Begin
         For iLayer := 0 to Soil.nLayers do
             LayerSum[iLayer, N] := Soil.FineWood[iLayer, N] +
                                 Soil.CoarseWood[iLayer, N];
         LayerOut(LayerSum, N, Control.OutputByLayer, 1);
         End;
      If SaveVar.Choose[S_CActive] then LayerOut(Soil.Active, C, Control.OutputByLayer, 0.001);
      If SaveVar.Choose[S_NActive] then LayerOut(Soil.Active, N, Control.OutputByLayer, 1);
      If SaveVar.Choose[S_CSlow] then LayerOut(Soil.Slow, C, Control.OutputByLayer, 0.001);
      If SaveVar.Choose[S_NSlow] then LayerOut(Soil.Slow, N, Control.OutputByLayer, 1);
      If SaveVar.Choose[S_CResistant] then LayerOut(Soil.Resistant, C, Control.OutputByLayer, 0.001);
      If SaveVar.Choose[S_NResistant] then LayerOut(Soil.Resistant, N, Control.OutputByLayer, 1);
      If SaveVar.Choose[S_CInert] then LayerOut(Soil.Inert, C, Control.OutputByLayer, 0.001);
      If SaveVar.Choose[S_NInert] then LayerOut(Soil.Inert, N, Control.OutputByLayer, 1);
      If SaveVar.Choose[S_NMineral] then LayerOut(Soil.Soluble, N, Control.OutputByLayer, 1);
      If SaveVar.Choose[S_StoredWater] then
         If Control.OutputByLayer then
            For iLayer := 0 to Soil.nLayers do
                DatumOut(Soil.WaterLayer[iLayer].WaterContent, 1)
         Else
            Begin
            DatumOut(Soil.WaterLayer[0].WaterContent, 1);
            Sum := 0;
            For iLayer := 1 to Soil.nLayers do
                Sum := Sum + Soil.WaterLayer[iLayer].WaterContent;
            DatumOut(Sum, 1);
            End;
      If Control.IncludeP then
         Begin
         ...
         If SaveVar.Choose[S_RemovedP] then DatumOut(Derived.Removed[P], 1);
         ...
         If SaveVar.Choose[S_NecroP] then
            ...
            End;
         If SaveVar.Choose[S_MetabolicP] then LayerOut(Soil.Metab, P, Control.OutputByLayer, 1);
         If SaveVar.Choose[S_StructuralP] then LayerOut(Soil.Struct, P, Control.OutputByLayer, 1);
         If SaveVar.Choose[S_WoodyLitterP] then
            Begin
            For iLayer := 0 to Soil.nLayers do
                LayerSum[iLayer, P] := Soil.FineWood[iLayer, P] +
                                 Soil.CoarseWood[iLayer, P];
            LayerOut(LayerSum, P, Control.OutputByLayer, 1);
            End;
         If SaveVar.Choose[S_ActiveP] then LayerOut(Soil.Active, P, Control.OutputByLayer, 1);
         If SaveVar.Choose[S_SlowP] then LayerOut(Soil.Slow, P, Control.OutputByLayer, 1);
         If SaveVar.Choose[S_ResistantP] then LayerOut(Soil.Resistant, P, Control.OutputByLayer, 1);
         If SaveVar.Choose[S_InertP] then LayerOut(Soil.Inert, P, Control.OutputByLayer, 1);
         If SaveVar.Choose[S_RockP] then LayerOut(Soil.RockP, P, Control.OutputByLayer, 1);
         If SaveVar.Choose[S_OccludedP] then LayerOut(Soil.OccludedP, P, Control.OutputByLayer, 1);
         If SaveVar.Choose[S_2ndaryP] then LayerOut(Soil.SecondaryInorganicP, P, Control.OutputByLayer, 1);
         If SaveVar.Choose[S_MineralP] then LayerOut(Soil.Soluble, P, Control.OutputByLayer, 1);
         End;
