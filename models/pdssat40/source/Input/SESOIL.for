...
C           PATHSL,SWINIT,INO3,INH4,ISWNIT,ISWWAT
C
C  LOCAL  :
C
C  OUTPUT :
...
C  Called : SENS
C
C  Calls  : CLEAR IPSOIL SEINIT CLEAR SEDLYR IPSLIN IPSLAN SEPROF SESURF
C-----------------------------------------------------------------------
C                         DEFINITIONS
C
C  HDLAY  :
C=======================================================================
      ...
      SUBROUTINE SESOIL (FILES,FILEX,NSENS,RNMODE,SLNF,SLPF,U,SWCON,
     &     CN2,SALB,DEPMAX,PEDON,SLNO,SLTX,LL,DUL,SAT,WR,BD,OC,PH,DLAYR,
     &     NLAYR,DS,LNIC,LNSA,YRIC,PRCROP,WRESR,WRESND,EFINOC,EFNFIX,
     &     PATHSL,SWINIT,INO3,INH4,ISWNIT,ISWWAT,EXTP,
     &     ICWD,ICRES,ICREN,ICREP,ICRIP,ICRID,SWCN,ADCOEF,TOTN,YRSIM)

      USE ModuleDefs
      IMPLICIT     NONE

      CHARACTER*1  RNMODE,BLANK,SWSPRF,ISWWAT,ISWNIT
      CHARACTER*2  PRCROP
      CHARACTER*5  SLTX
      CHARACTER*6  ERRKEY
     ...
     &                   ICREP,ICRIP,ICRID)
      ...
      ELSE IF (MENU .EQ. 3) THEN
          ...
          IF (PATHL .LE. 1) THEN
           ...
           ELSE
             IF (PATHL .GT. 1) THEN
C-SUN           IF (PATHSL((PATHL-1):(PATHL-1)) .NE. '/') THEN
C-SUN              WRITE (PATHSL(PATHL:PATHL),'(A1)') '/'
                IF (PATHSL((PATHL-1):(PATHL-1)) .NE. '\') THEN
                   WRITE (PATHSL(PATHL:PATHL),'(A1)') '\'
                   PATHL = PATHL+1
                ENDIF
             ENDIF
             FILESS = PATHSL (1:(PATHL-1)) // FILES
      ...
      ELSE IF (MENU .EQ. 4) THEN
          ...
          CALL IPSOIL (RNMODE,FILES,PATHSL,NSENS,ISWWAT)
          NSENS = 1
          CALL IPSLIN (FILEX,LNIC,NLAYR,DUL,YRIC,PRCROP,WRESR,WRESND,
     &         EFINOC,EFNFIX,PEDON,SLNO,DS,SWINIT,INH4,INO3,
