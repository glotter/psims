...
C-JED     IF (PATHX(IPX-3:IPX-3) .EQ. '.') THEN
C-JED         ERRORX = PATHX(1:(IPX-12)) // 'MODEL.ERR   '
C-JED       ELSE
C-JED         ERRORX = PATHX(1:(IPX-8)) // 'MODEL.ERR   '
C-JED     ENDIF
...
!     If ERRORX file is not in executable directory, try std. location
      ...
      IF (.NOT. FEXIST) THEN
        SAVE_ERRORX = ERRORX
        ERRORX = '/home/jelliott/dssat_build/MODEL.ERR'
      ENDIF
...
C-JED   DO WHILE (.NOT. EOF(LUN))
        DO WHILE (istat.eq.0 )
          READ(LUN, '(I4)', iostat=istat) LUNIT
