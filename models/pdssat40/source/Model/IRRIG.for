...
!      CHARACTER*70 IrrText
      ...
      INTEGER YRDIF
      INTEGER IDLAPL(NAPPL), IRRCOD(NAPPL)
      INTEGER JULAPL(NAPPL), JULWTB(NAPPL), JWTBRD(NAPPL)
      ...
      REAL AMIR(NAPPL), AMT(NAPPL), WTABL(NAPPL)
...
C-----------------------------------------------------------------------
          ...
          ELSE
            READ(LUNIO,'(/,14X,2(1X,F5.0),18X,2(1X,F5.0))',
     &        IOSTAT=ERRNUM) DSOIL, THETAC, AIRAMT, EFFIRR
            LNUM = LNUM + 2
...
!     soil organic matter.  This is not available currently.
      MESOM = ISWITCH % MESOM
...
C-----------------------------------------------------------------------
     ...
     &      THEN
      ...
      CUMDEP = 0.
      DO I = 1, NLAYR
         CUMDEP = CUMDEP + DLAYR(I)
      END DO

!     Adjust IPERC values if necessary.
      DO I = 1, NPERC
        IF (CUMDEP .LE. 20.0 .AND. IPERC(I) .GT. 0.) THEN
          IPERC(I) = IPERC(I) * 20.
        ENDIF
        IF (IPERC(I) .LE. 0.0) THEN
          IF (CUMDEP .LE. 20.) THEN
