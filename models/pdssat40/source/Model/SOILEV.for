...
!-----------------------------------------------------------------------
!  Set air dry water content for top soil layer (from INSOIL)
!-----------------------------------------------------------------------
...
C  01/15/2003 KJB CHANGED TO USE SAME K EXTINCTION AS USED FOR TRANSP
C    VILLALOBOS MEASURED K=0.52 FOR BOTH SOIL EVAPORATION AND TRANSP
C    THEORY OF GOUDRIAAN INDICATES A VALUE NEAR 0.5 BECAUSE OF BOTH
C    NEAR-INFARED PLUS VISIBLE COMBINE TO GIVE YOU THIS COMBINATION K
...
!     Old computation:
        IF (XHLAI .LE. 1.0) THEN   !<-------!
          EOS = EO*(1.0 - 0.43*XHLAI)       !
        ELSE                                !-> old code
          EOS = EO/1.1*EXP(-0.4*XHLAI)      !
        ENDIF     !<------------------------! 

      ELSE
        EOS = EO*EXP(-KSEVAP*XHLAI) !<------- Tony's new code 07/02/2003
      ENDIF


!     Ken's new computation: 01/03/2003
!      EOS = EO*EXP(-0.50*XHLAI)
...
!  Calls:       None
C=======================================================================
      SUBROUTINE FLOOD_EVAP(XHLAI, EO, EF)

!-----------------------------------------------------------------------
      IMPLICIT NONE
      REAL EO, EF, EF85, XHLAI

!-----------------------------------------------------------------------
        EF = EO*(1.0-0.45*XHLAI)
      IF (XHLAI .GT. 0.85) THEN
        EF85 = EO * 0.62  !EF at XHLAI=0.85, prevents discontinuity
        EF = AMIN1(EF85, EO/1.1*EXP(-0.60*XHLAI))
      ENDIF
