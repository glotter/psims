...
C=======================================================================
     ...
     &    ALGFIX, BD1, CUMFNRO, TOTAML, TOTFLOODN)        !Output
      ...
      REAL ALI, ALGACT, ALGFIX, BD1, BDP, BD2, BDP2, DLAYR1, EF
     ...
     &    DLTFON, DLTSNH4, DLTSNO3, DLTUREA, 
     &    DUL, LL, FON, OC, PH, PO, 
     &    SAT, SNO3, SNH4, SW, 
     &    UREA, WFP
      ...
      REAL OXU, OXH4, OXN3
      LOGICAL AKILL, DAILY, IMMOBIL, PUDDLED
...
!     Zero flood N flux variables
     ...
     &    FLOOD, FON, LFD10, NSWITCH, OC, OXMIN3, OXMIN4, !Input
     &    OXU, OXH4, OXN3, OXLT, PH, SNH4, SNO3, SRAD,    !Input
     &    SW1, TMAX, TMIN, UREA, XHLAI, YRDOY, YRDRY,     !Input
     &    AKILL, DAILY, DLTFON, DLTSNH4, DLTSNO3, DLTUREA,!I/O
     &    DLTFUREA, DLTFNO3, DLTFNH4, DLTOXU, DLTOXH4,    !I/O
     &    DLTOXN3, IBP, IMMOBIL,                          !I/O
...
!     From POROSITY subroutine
     ...
     &    CUMFNRO, FNRO)                                  !Output
      ...
      !FAC(1) = 1.0/(BD1*1.E-01*DLAYR(1))
      !FAC(2) = 1.0/(BD2*1.E-01*DLAYR(2))
...
!     If flood waters just dryed up, perform initialization
      IF (YRDOY .EQ. YRDRY) THEN
        CALL DRYUP(
     &    LFD10, YRDOY,                                   !Input
     &    AKILL, ALGFIX, DLTFON, DLTOXN3, DLTOXH4,        !I/O
...
!     If newly flooded conditions exist, perform initialization
     ...
     &    DAILY, DLTUREA,  DLTSNH4, DLTSNO3, DLTFUREA,    !I/O
...
C=======================================================================
     ...
     &    ALGACT, DAILY)                                  !Output
      ...
      REAL    ALGFIX
      REAL, DIMENSION(NL) :: DLTFON, DLTSNH4, DLTSNO3, DLTUREA
      LOGICAL DAILY
...
!     Floodwater N is lost from the system.
      ...
      DLTFUREA = DLTFUREA - FRACLOSS * FLDU
      DLTFNO3  = DLTFNO3  - FRACLOSS * FLDN3
      DLTFNH4  = DLTFNH4  - FRACLOSS * FLDH4
