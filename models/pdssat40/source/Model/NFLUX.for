...
!       get a positive value of NDOWN, because FLOW is negative (thus
...
!       DLTN it will be given a sign.
     ...
     &      ABS (FLOW(L)) / (SW(L) * DLAYR(L) + ABS (FLOW(L)))

!       Upward N flow out of this layer (does not apply to layer 1).
        ELSEIF (FLOW(L) .GT. 0 .AND. L .GT. 1) THEN
          NUP(L) = AMAX1 (NORIG(L) * FRAC_SOLN(L), 0.) *
     &      FLOW(L) / (SW(L) * DLAYR(L) + FLOW(L))
        ENDIF   !End of IF block on FLOW.
...
! FLOW(L)        Volume of water moving from soil layer L by unsaturated 
