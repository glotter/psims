...
C=======================================================================
     ...
     &    CONTROL, FILEW, XLAT, YYDDD,                    !Input
     &    DAYL, PAR, RAIN, SRAD, TDEW,                    !Input/Output
     &    TMAX, TMIN, TWILEN, WINDSP,                     !Input/Output
     &    CO2, DEC, NEV, SNUP, SNDN)                      !Output
...
!-----------------------------------------------------------------------
      ...
      INTEGER DYNAMIC,MULTI,YRSIM,YR,WDATE
...
C     Change: additive <> 0, multiplicative <> 1, constant > 0.
      ...
      IF (TYPEC .EQ. 'A' .AND. ABS(AMOUNT) .GT. 1E-5) THEN
!     Add AMOUNT to weather value
        WTHADJ(1,VNUM) = AMOUNT
        WTHADJ(2,VNUM) = 1.
        WMODB = 'Y'

      ELSEIF (TYPEC .EQ. 'S' .AND. ABS(AMOUNT) .GT. 1E-5) THEN
!     Subtract AMOUNT from weather value
        WTHADJ(1,VNUM) = -AMOUNT
        WTHADJ(2,VNUM) = 1.
        WMODB = 'Y'

      ELSE IF (TYPEC .EQ. 'M' .AND. ABS(AMOUNT - 1.0) .GT. 1E-5) THEN
