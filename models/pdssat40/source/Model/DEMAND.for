...
C-----------------------------------------------------------------------
      IF (DAS .EQ. NR1) THEN
!-----------------------------------------------------------------------
C     Fraction of growth going to leaves and roots decreases
...
!-----------------------------------------------------------------------
C=======================================================================
C  IPDMND, Subroutine, C.H. Porter
C-----------------------------------------------------------------------
C  Reads input data for DEMAND subroutine
C-----------------------------------------------------------------------
C  REVISION       HISTORY
...
C-----------------------------------------------------------------------
      ...
      IF (FOUND .EQ. 0) THEN
        CALL ERROR(SECTION, 42, FILEIO, LNUM)
      ELSE
        READ(LUNIO,'(24X,A6,48X,3F6.0)',IOSTAT=ERR) 
     &      ECONO, SLAVAR, SIZELF, XFRUIT   ;    LNUM = LNUM + 1
