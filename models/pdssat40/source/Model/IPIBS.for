...
C-----------------------------------------------------------------------
      ...
      CHARACTER*1  ISWWAT, ISWNIT, ISWCHE, ISWTIL
      CHARACTER*1  ISWSYM, ISWPHO, ISWPOT, ISWDIS
      CHARACTER*1  MEEVP, MEPHO, MESIC, MESOM
...
!     FILEX used to determine output file names when FNAME <> 'OVERVIEW'
      ...
      READ (LUNIO,'(15X,A12)', IOSTAT=ERRNUM) FILEA    
      LNUM = LNUM + 1 
      IF (ERRNUM .NE. 0) CALL ERROR(ERRKEY,ERRNUM,FILEIO,LNUM)
...
C-----------------------------------------------------------------------
        READ (LUNIO,'(2(/),14X,9(5X,A1))', IOSTAT=ERRNUM) 
     &   ISWWAT, ISWNIT, ISWSYM, ISWPHO, ISWPOT, ISWDIS, ISWCHE, ISWTIL
        LNUM = LNUM + 3 
        IF (ERRNUM .NE. 0) CALL ERROR(ERRKEY,ERRNUM,FILEIO,LNUM)
        ...
        READ (LUNIO,200, IOSTAT=ERRNUM) MESIC, MEEVP, MEPHO, NSWI, MESOM
  200   FORMAT(25X, A1, 11X, A1, 11X, A1, 11X, I1, 5X, A1)
        LNUM = LNUM + 1 
        IF (ERRNUM .NE. 0) CALL ERROR(ERRKEY,ERRNUM,FILEIO,LNUM)

        IF (MEEVP .EQ. 'Z' .AND. MEPHO .NE. 'L') THEN
          ! If Canopy photosynthesis method is used, then zonal
          ! evapotranspiration method cannot be used.  Set default to 
          ! Penman method.
          MEEVP = 'P' 
        ENDIF
...
C-----------------------------------------------------------------------
      ...
      IF (MEPHO .NE. 'C' .AND. INDEX(MODEL,'CRGRO') .LE. 0) THEN
        MEPHO = 'C'
        WRITE (MESSAGE(1),210) MODEL
        WRITE (MESSAGE(2),220)
  210   FORMAT("Method of photosynthesis invalid for model ",A8,".")
  220   FORMAT("Changed to Canopy photosynthesis.") 
...
! ISWTIL   Swith for observed tillage application 
...
! OUTC     Carbon summary output file (e.g., CARBON.OUT) 
! OUTD     File name for pest damage output file (e.g., PEST.OUT) 
! OUTE     Environmental data output file (e.g., ENVIRON.OUT) 
! OUTG     Growth output file name (typically 'GROWTH.OUT') 
! OUTH     Chemical summary output file (e.g., CHEMICAL.OUT) 
! OUTN     Nitrogen summary output file (e.g., NITROGEN.OUT)
! OUTO     Overview of simulation output file (e.g., OVERVIEW.OUT) 
! OUTR     Operations management output file (e.g., OPERAT.OUT) 
! OUTS     Simulation summary file (e.g., SUMMARY.OUT) 
! OUTW     Filename for soil water output file, e.g., WATER.OUT (set in 
!            IPIBS) 
