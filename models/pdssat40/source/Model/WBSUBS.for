...
!-----------------------------------------------------------------------
      ...
      INTEGER NLAYR, RUN
      INTEGER ERRNUM, FOUND, L, LINC, LNUM
...
!     If this is a sequenced run (MESIC = 'S') skip this section
C-----------------------------------------------------------------------
!      IF (MESIC .EQ. 'M') THEN
!     CHP 6/14/2005 Use RNMODE as indicator of sequence mode.
        ...
        ELSE
          ...
          DO L = 1, NLAYR
            ...
            IF (SW(L) .LT. LL(L)) THEN
              SW(L) = LL(L)
 !  This call to WARNING results in closing FILEIO (in HEADER routine).
 !             WRITE(MSG(1),10) L
 !             CALL WARNING(1,ERRKEY,MSG)
 !  10 FORMAT('Soil layer ',I2,': Initial soil water content set to ',
 !    &                'lower limit.')
            ENDIF
          ENDDO
...
!-----------------------------------------------------------------------
      ...
      IF (ERRNUM .NE. 0) CALL ERROR(ERRKEY,ERRNUM,FILEIO,0)
C-----------------------------------------------------------------------
C    Find and Read Initial Conditions Section - only for seasonal runs
!     where MULTI > 1
C-----------------------------------------------------------------------
      SECTION = '*INITI'
...
! MEHYD   Hydrology simulation switch: R=RITCHIE 
...
C  10/13/97 CHP Modified for modular format
C  09/01/99 GH  Incorporated into CROPGRO
!-----------------------------------------------------------------------
!  Called by: WATBAL
!  Calls:     None
C=======================================================================
...
!     NL defined in ModuleDefs.for
      ...
      REAL DLAYR(NL), DUL(NL), ESW(NL), FLOW(NL), LL(NL)
...
!      Upward flow from layer M to layer L
        IF (FLOW(L) .GT. 0.0) THEN
          IF (SWTEMP(L) .LE. DUL(L)) THEN
            SWTEMP(L) = SWTEMP(L) + FLOW(L) / DLAYR(L)
            SW_INF(L) = SW_INF(L) + FLOW(L) / DLAYR(L)
            IF (SWTEMP(L) .GT. DUL(L) .OR. SW_INF(L) .GT. SAT(L)) THEN
              FLOWFIX = MAX(0.0, (SWTEMP(L) - DUL(L)) * DLAYR(L), 
     &                           (SW_INF(L) - SAT(L)) * DLAYR(L))
              FLOWFIX = MIN(FLOW(L), FLOWFIX)
              FLOW(L) = FLOW(L) - FLOWFIX
              SWTEMP(L) = SWOLD + FLOW(L) / DLAYR(L)
            ENDIF
          ELSE    !No upward flow if SWTEMP > DUL
            FLOW(L) = 0.
          ENDIF

          IF (FLOW(L) / DLAYR(M) .GT. SW_AVAIL(M)) THEN
            FLOW(L) = SW_AVAIL(M) * DLAYR(M)
            SWTEMP(L) = SWOLD + FLOW(L) / DLAYR(L)
          ENDIF
          SWTEMP(M) = SWTEMP(M) - FLOW(L) / DLAYR(M)

!-----------------------------------------------------------------------
!       Downward flow from layer L to layer M
        ELSE IF (FLOW(L) .LT. 0.0) THEN
          IF (SWTEMP(L) .GE. LL(L)) THEN
            IF (ABS(FLOW(L) / DLAYR(L)) .GT. SW_AVAIL(L)) THEN
              FLOW(L) = - SW_AVAIL(L) * DLAYR(L)
            ENDIF
              
            SWTEMP(L) = SWTEMP(L) + FLOW(L) / DLAYR(L)
            SWTEMP(M) = SWTEMP(M) - FLOW(L) / DLAYR(M)
            SW_INF(M) = SW_INF(M) - FLOW(L) / DLAYR(M)

            IF (SW_INF(M) .GT. SAT(M)) THEN
              FLOWFIX = MIN(ABS(FLOW(L)), 
...
!-----------------------------------------------------------------------
!     UPFLOW VARIABLE DEFINITIONS: updated 2/19/2004
!-----------------------------------------------------------------------
! DBAR         
...
!  10/20/97 CHP Modified for modular format.
!  03/17/01 CHP Allows water table between top and bottom of layer
...
!-----------------------------------------------------------------------
!     Initially set water table at bottom of profile
      WTDEP = DS(NLAYR)

      IF (SW(1) .GE. SAT(1)) THEN
!       Top layer saturated. Water table at surface.
        WTDEP = 0.0
      ELSE
        DO L = 1, NLAYR-1
          IF (SW(L+1) .GE. SAT(L+1)) THEN
!            Layer below is saturated.  Interpolate water table depth.
             SATFRAC = (SW(L) - DUL(L)) / (SAT(L) - DUL(L))
             SATFRAC = MIN(MAX(0.0, SATFRAC), 1.0)
             WTDEP = DS(L) - DLAYR(L) * SATFRAC
             GOTO 100
         ENDIF
      ENDDO
      ENDIF
