...
!  11/07/2005 CHP Fixed check for LDF10
C=======================================================================
...
C  UNINCO : Fertilizer not fully incorporated -  If true, ignore oxlayer
      ...
      !
      INTEGER  FERTYPE
      REAL     UREA(NL),SNH4(NL),SNO3(NL),SRAD,SALB,XHLAI
...
!-----------------------------------------------------------------------
      ...
      IF (INDEX('FQ',RNMODE) .LE. 0 .OR. RUN .EQ. 1) THEN
        ...
        OXU   = UREA(1) * FAC(1) / OXFAC
        OXN3  = SNO3(1) * FAC(1) / OXFAC
        OXH4  = SNH4(1) * FAC(1) / OXFAC
        OXNI  = 0.05
...
!      ENDIF
      ...
      !
      ! OXFAC is like Fac(L) in layered routines
      !
...
!         IF (IHD .EQ. 1) THEN
         ...
         DO I = IST, IHR
            ...
            ENDIF
            !
! Calculate ammonia (Mass) using inverse of (Fac) OXLT in cm)
            !
