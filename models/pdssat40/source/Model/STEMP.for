...
!TEMP CHP
! USE OLD (INCORRECT) METHOD FOR WC CALCULATION
      METMP = 1

! USE NEW (CORRECTED) METHOD FOR WC CALCULATION
      !METMP = 3
...
!        This is done in weather routine.
!        MSGCOUNT = 0
!        IF (TAV  .LE. 0.0) THEN
!          TAV = 20.0
...
!-----------------------------------------------------------------------
!     CHP 11/25/2003 LAH found possible error in computation of WC.
!     I made some comparisons with old and new WC equations 
!     for wheat, soybean, peanut, potato and maize.  For all
!     crops except peanut, there were some significant differences 
!     to yields as a result of the corrected soil temperature function. 
!     For this reason, the revision will not be made permanent
!     until data sets can be evaluated and recalibrated if necessary.
