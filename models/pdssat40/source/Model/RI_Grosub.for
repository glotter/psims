...
C=======================================================================
     ...
     &    FLOOD, ISTAGE, ITRANS, LTRANS, NEW_PHASE,       !Input
     &    NH4, NO3, P1, P1T, P3, P4, PHEFAC, RLV, RTDEP,  !Input
     &    SDEPTH, SDTT_TP, SI3, SOILPROP, SRAD, ST,       !Input
     &    STRCOLD, STRESSW, STRHEAT, SUMDTT, SW, SWFAC,   !Input
     &    TAGE, TBASE, TF_GRO, TMAX, TMIN, TSGRWT,        !Input
     &    TURFAC, WSTRES, XSTAGE, XST_TP, YRPLT, YRSOW,   !Input
     &    EMAT, FLOODN, PLANTS, RTWT,                     !I/O
     &    AGEFAC, APTNUP, BIOMAS, CANNAA, CANWAA, DYIELD, !Output
     &    GNUP, GPP, GPSM, GRAINN, GRNWT, GRORT, LAI,     !Output
     &    LEAFNO, LFWT, MAXLAI, NSTRES, PANWT, PBIOMS,    !Output
     &    PHINT, PLTPOP, PORMIN, RLWR, ROOTN, RWUEP1,     !Output
     &    RWUMX, SEEDNI, SEEDRV, SKERWT, STMWT, STOVER,   !Output
     &    STOVN, TANC, TGROGRN, TILNO, TOTNUP,            !Output
     &    UNH4, UNO3, XGNP)                               !Output
...
!-----------------------------------------------------------------------
      ...
      REAL SDEPTH, SDTT_TP
      REAL SDWTPL, SEEDNI, SEEDRV, SENLA, SHOCKFAC, SKERWT
      REAL SLAN, SNLFWT, SPACE, SPIKE, SRAD, STMWT, STOVER, STOVN 
...
!          PANNO  = (TILNO+1.0)*PLANTS    !Not used
     ...
     &    TSHOCK)                                         !Output
      ...
      CARBO = PCARB*AMIN1(PRFT,SWFAC,NSTRES,TSHOCK)
...
!        PC = 0.56 + 0.068*CUMPH  ! J.Padilla
     ...
     &            AMIN1(TURFAC,TEMF,AGEFAC)
          ...
          IF (SEEDRV .LT. 0.0) THEN
             ...
             MGROLF = CARBO*0.65
             GRORT  = CARBO - MGROLF
             MPLAG  = MGROLF/0.0055*TSHOCK*AMIN1(TURFAC,TEMF,AGEFAC)
          ENDIF
          ...
          IF (CUMPH .GT. 5.0/G3 .AND. TSHOCK .GE. 1.0) THEN
             IF (MGROLF .GT. 0.60*CARBO) THEN
                MGROLF = 0.60*CARBO
                MPLAG  = MGROLF/0.0060*AMIN1 (TURFAC,TEMF,AGEFAC)
             END IF
             GRORT = 0.25*CARBO
     ...
     &             AMIN1(TURFAC,TEMF,AGEFAC)*TSHOCK
          ...
          IF (CUMPH .LE. 6.0) THEN
             IF (MGROLF .GT. CARBO*0.60) THEN
                MGROLF = CARBO*0.60
                MPLAG  = MGROLF/0.0060*TSHOCK*AMIN1(TURFAC,TEMF,AGEFAC)
             ENDIF
             MGROSTM = MGROLF * 0.15
     ...
     &            TILNO, TLFWT, TSTMWT, TPLA)                    !Output
        ...
        CASE (3)  ! Heading and end of leaf growth
          MPLAG   = A/CUMPH*(-BF)*XTN*EXP(XTN)*TI*
     &              AMIN1(TURFAC,TEMF,AGEFAC)+(0.65*TCARB1)/0.0055
          GRORT   = CARBO*0.15
          MGROLF  = MPLAG*0.0060
