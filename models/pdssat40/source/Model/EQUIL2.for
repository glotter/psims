...
!         EQUIN = OXSPEC + SSPEC
      ...
      IF (DIFFN .LT. 0.0) THEN
         !
         ! From oxlayer to bulk soil
         !
         IF ((DIFFN + OXSPEC - OXMIN) .LT. 0.0) THEN
            DIFFN = OXMIN - OXSPEC
         ENDIF
       ELSE
         !
...
C
      ...
      IF (C .GT. -0.1 .AND. C .LT. 0.0) THEN
         C    = 0.0
         SPPM = SMINC
      ENDIF
      IF (C .LT. 0.00001 .AND. C. GT. 0.0) THEN
         C = 0.0
      ENDIF
      IF (C .EQ. 0.0) THEN
         SOLN  = 0.0
