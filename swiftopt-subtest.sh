...
# write runtask in run directory
cat << END > runtask
#!/bin/bash
variables=$variables
...
lon_zero=$lon_zero
out_file=$out_file
../bin/append.sh "$variables" "$long_names" "$var_units" $num_lons $num_lats $delta $num_years $scens $ref_year $lat_zero $lon_zero ./parts $out_file
END
chmod +x runtask 
